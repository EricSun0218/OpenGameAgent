    ...
    IAsyncDisposable
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using var control = Controls.Register(requestSnapshot.Run.RunId);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await using var ownership = await _ownership.AcquireAsync(
                ...
                cancellationToken)
            .ConfigureAwait(false);
        using var control = Controls.Register(runId);
    ...
    public ValueTask StopAsync()
    {
        ...
        if (initiate)
        {
            _ = CancelAndDisposeDetachedAsync(_shutdownCancellation);
            _ = CompleteStopAsync(
                drainTask!,
                completion!);
        }
        ...
        RunDeadline deadline)
    {
        ...
        while (true)
        {
            ...
            var snapshot = new TurnSnapshot
            {
                ...
                StablePrefixHash = RuntimePromptBuilder.TranscriptDigest(
                    preparedTranscript,
            ...
            foreach (var call in valid)
            {
                ...
                if (executor.TryGetReceipt(call.ToolCallId, out var actual))
                {
                    receipt = actual!;
                }
                else
                    ...
                    var failed = executionResults?
                        .FirstOrDefault(
                            item => string.Equals(
                                ...
                                call.ToolCallId,
                                StringComparison.Ordinal));
                    var errorCode = failed?.ErrorCode
                        ?? (executionCancelled
                            ? "tool_execution_cancelled"
                            : "tool_result_missing");
                    receipt = failed is not null
                              && !failed.MayHaveExecuted
                        ? FailedReceipt(
    ...
    {
        var toolDeadline = _clock.UtcNow.AddMilliseconds(call.Tool.TimeoutMs);
        var request = new ActionRequest
        {
            ...
            Deadline = toolDeadline < runDeadline
                ? toolDeadline
                : runDeadline
        };
    ...
    private static DurableRunRequest Snapshot(DurableRunRequest request)
    {
        var run = request.Run
        ...
        return new DurableRunRequest
        {
            ...
            Context = context.Select(Snapshot).ToArray(),
            ActiveSkills = activeSkills.Select(Snapshot).ToArray(),
    ...
    private static DurableRunContinuation Snapshot(
    ...
    private static Task CancelAndDisposeDetachedAsync(
        CancellationTokenSource cancellation)
    {
        return Task.Run(
            () =>
            {
                ...
                {
                    // Runtime shutdown is already committed. An untrusted
                    // callback cannot fault or delay the StopAsync caller.
                }
                finally
                {
    ...
    private sealed class RunDeadline : IDisposable
    {
        ...
        public bool IsExpired =>
            _timer.IsCancellationRequested
        ...
        public void Dispose()
        {
            ...
            var lifetimeCancellation =
                CancelWithoutSurfacingCallbacksDetached(_lifetime);
            ...
            CancellationToken lifetime)
        {
            try
            {
                ...
                await CancelWithoutSurfacingCallbacksDetached(timer)
                    .ConfigureAwait(false);
            }
        ...
        private static Task CancelWithoutSurfacingCallbacksDetached(
            CancellationTokenSource source)
        {
            return Task.Run(
                () =>
                {
                    ...
                    {
                        // Third-party callbacks cannot fault the deadline
                        // timer or its owning run.
                    }
                });
        }
    }
