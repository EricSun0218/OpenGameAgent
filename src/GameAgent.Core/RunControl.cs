    ...
    private BufferedCommand Snapshot(RunControlCommand command)
    {
        ...
        if (command.Observation is not null)
        {
            var limits = new JsonValueLimits(
                maxUtf8Bytes: _options.MaxObservationUtf8Bytes,
                maxStringUtf8Bytes: Math.Min(
                    _options.MaxObservationUtf8Bytes,
                    65_536));
            var safeObservation =
            ...
            observationBytes = JsonValueInspector.ValidateAndMeasure(
                encoded,
                limits,
                nameof(command));
            observation = ProtocolJson.DeserializeObservationEnvelope(
                encoded.GetRawText());
        }
