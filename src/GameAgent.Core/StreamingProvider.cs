...
public sealed class ProviderRouteMetadata
{
        ...
        string transportDialect)
    {
        ModelId = RuntimeGuard.RequiredUtf8(
            ...
            256,
            nameof(modelId));
        TransportDialect = RuntimeGuard.RequiredUtf8(
            transportDialect,
            128,
            nameof(transportDialect));
    }
...
{
    internal ProviderRouteIdentity(
        string providerId,
...
public sealed class ProviderAttemptRunner
{
        ...
        ProviderStreamLimits? streamLimits = null)
    {
        ...
        var stableRouteMetadata =
            new List<ProviderRouteMetadata>(providers.Count);
        foreach (var provider in providers)
        {
            ...
            stableRouteMetadata.Add(
                new ProviderRouteMetadata(
                    metadata.ModelId,
                    metadata.TransportDialect));
        }

        _providers = providers.ToArray();
        _providerIds = stableProviderIds.ToArray();
        _routeMetadata = stableRouteMetadata.ToArray();
