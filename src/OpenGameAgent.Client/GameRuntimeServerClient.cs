...
public sealed class GameRuntimeServerClient
{
    ...
    public GameRuntimeServerClient(GameRuntimeServerClientOptions options)
    {
        ...
        ValidateBaseUri(options.ServerBaseUri, options.AllowInsecureHttp);
    ...
    private static bool ContainsInvalidCredentialCharacters(string? value, int maximumCharacters) =>
        (value?.Length ?? 0) > maximumCharacters
    ...
    private static void ValidateBaseUri(Uri value, bool allowInsecureHttp)
    {
        if (value is null
            || !value.IsAbsoluteUri
        ...
        {
            throw new ArgumentException("The Runtime server base URI must be an absolute HTTP or HTTPS URI.", nameof(value));
        }
        ...
        if (string.Equals(value.Scheme, Uri.UriSchemeHttp, StringComparison.OrdinalIgnoreCase)
            ...
            && !allowInsecureHttp)
        {
            throw new ArgumentException("Remote Runtime servers must use HTTPS.", nameof(value));
        }
    }

    private static Uri CreateEndpoint(Uri baseUri, string path, string parameterName)
    {
        if (string.IsNullOrWhiteSpace(path) || !Uri.TryCreate(path, UriKind.Relative, out _))
        {
            throw new ArgumentException("A Runtime endpoint path must be relative.", parameterName);
        }
        ...
        {
            throw new ArgumentException("A Runtime endpoint cannot change the configured server origin.", parameterName);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidDataException("The Runtime server response exceeded its configured size limit.");
        }
            ...
            while (true)
            {
                ...
                for (var index = 0; index < read; index++)
                {
                    ...
                    else
                    {
                        ...
                        {
                            throw new InvalidDataException("A Runtime SSE line exceeded its configured size limit.");
                        }
                    }
                }
            }
    ...
    private static void EnsureSuccess(HttpResponseMessage response, string body)
    {
        if (!response.IsSuccessStatusCode)
        {
            throw new HttpRequestException(
                $"The Runtime server returned HTTP {(int)response.StatusCode} ({response.ReasonPhrase}). {body}");
        }
    }
}
