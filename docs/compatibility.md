...
| Native text/tool providers | OpenAI Responses and Gemini Interactions streaming APIs | Deterministic wire fixtures cover structured input, complete tool-call/result replay, text/tool deltas, usage normalization, terminal state, truncation, authentication failure, and cross-dialect canonical output |
...
The bundled adapters target the native OpenAI Responses, Gemini Interactions,
and Anthropic Messages APIs plus streaming chat-completions APIs that use a
configured OpenAI-compatible event shape. Providers vary in authentication,
streaming details, tool-call behavior, and usage reporting. Each configured
provider must be exercised before shipping a game. A provider token belongs in
a server relay or a short-lived scoped credential for consumer builds. See
[native model providers](native-model-providers.md).

The native OpenAI and Gemini adapters resend the runtime-owned normalized
history with provider storage disabled. Tool calls and results are represented
in each provider's native item or step format and return through one canonical
event stream. Both routes explicitly decline provider-private reasoning-input
replay and stateful continuation; reasoning effort and produced summaries are
separate declared capabilities. Exact request bytes are hashed as non-content
wire evidence before transport. Remote endpoints require HTTPS, redirects are
rejected, and SSE parsing is bounded and strict UTF-8.

`ProviderModelCatalog` captures the exact configured provider/model routes and
their dialect, capability, and route digests. `Evaluate` reports stable missing
capability codes and `Select` returns only full matches. A declared numeric
limit of zero is unspecified rather than unlimited.
