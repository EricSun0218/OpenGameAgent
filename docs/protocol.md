...
snapshots, action requests, and receipts are durable. Provider deltas and UI
progress are ephemeral. `assistant.delta` identifies normalized text,
tool-call, usage, and completion stream events; raw reasoning deltas are not
published to the general observer. `tool.progress` identifies its operation,
tool call, action, stage, optional message/counters, and optional structured
data. It is non-authoritative and cannot settle an action. A bounded
notification consumer may drop ephemeral events under backpressure. Durable
events remain available for replay from the journal; ephemeral events do not.
Consumers that need authoritative state must use the journal rather than the
live queue alone.
