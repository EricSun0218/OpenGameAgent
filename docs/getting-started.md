...
- An OpenAI Responses, Gemini Interactions, Anthropic Messages, compatible
  chat-completions endpoint, or an implementation of
...
For direct OpenAI Responses and Gemini Interactions configuration, capability
negotiation, and credential boundaries, see
[native model providers](native-model-providers.md). `built.Models` is the
immutable catalog of exact configured routes; `built.Inspector` queries and
exports authoritative run history. Neither API supplies game tools or game
rules: those still come from the host and its registered catalog.
