...
When generated bytes must become a persistent game asset, use `DurableGameMediaPipeline` rather than treating a provider response as a finished asset. The pipeline binds a stable operation to the complete session identity, model, importer, expected revision, and a fingerprint of the prompt, parameters, and source hashes; materializes bounded outputs into content-addressed storage; persists the manifest before import; and records the engine's authoritative receipt. An interrupted import is reconciled by its stable operation ID instead of repeated blindly. The game still owns the asset schema, moderation, licensing, quotas, engine-thread scheduling, and final save mutation. See [Generated assets](generated-assets.md).
