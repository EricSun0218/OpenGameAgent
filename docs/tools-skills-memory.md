...
code. When a host executor ignores cancellation, the scheduler returns
`tool_timeout` and keeps the original parallelism, conflict, and effect leases
until that executor actually finishes. It also places the execution in a
