...
- Add bilingual English-first project documentation, contributor governance,
  support and security policies, issue and pull-request templates, citation
  metadata, and an explicit threat model for the public open-source release.
- Add locked NuGet dependency graphs, SHA-pinned CodeQL, dependency-review, and
  OpenSSF Scorecard workflows, plus license-file validation in every NuGet
  package, deterministic package/archive verification, and a self-testing
  trusted-release workflow contract.
- Upgrade the test stack to xUnit v3 and the current test SDK, propagate test
  cancellation through asynchronous operations, and upgrade direct JSON and
  SQLite dependencies without suppressing analyzer findings.
- Add direct OpenAI Responses and Gemini Interactions streaming adapters with
  complete normalized tool-history translation, strict bounded SSE parsing,
  route-bound wire evidence, cross-provider conformance tests, and explicit
  inference-control rejection.
- Add an immutable configured-model catalog with capability negotiation and
  stable missing-capability diagnostics.
- Complete the live provider event payloads, add execution-scoped bounded game
  action progress, and expose a read-only runtime inspector with causal
  analysis, bounded queries, and redacted integrity-protected export.
