...
var routed_id := GameAgentRuntime.start_routed_run(
    route_dictionary,
    run_dictionary,
    observations,
    options,
    {})
var completion_id := GameAgentRuntime.start_completion({
...
```gdscript
var child_id := GameAgentRuntime.start_child_agent_run(
    parent_run_id,
    child_run,
    child_observations,
    child_options)
var cancelled := GameAgentRuntime.cancel_child_agent_runs(parent_run_id)
var later_child_id := GameAgentRuntime.start_child_agent_run_with_parent(
    persisted_parent_run,
    later_child_run,
    later_child_observations,
    later_child_options)
var run_cancelled := GameAgentRuntime.cancel_run(run_dictionary["runId"])
```
...
```gdscript
func _ready() -> void:
    GameAgentRuntime.run_completed.connect(_on_run_completed)
    GameAgentRuntime.run_failed.connect(_on_run_failed)
    GameAgentRuntime.runtime_error.connect(_on_runtime_error)
