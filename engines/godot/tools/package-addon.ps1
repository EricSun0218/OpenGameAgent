...
$archivePath = [IO.Path]::GetFullPath(
    (Join-Path $artifactsRoot "OpenGameAgent-godot-$Version.zip"))
$stagedArchivePath = [IO.Path]::GetFullPath(
    (Join-Path $artifactsRoot (
        ".OpenGameAgent-godot-$Version." +
        [Guid]::NewGuid().ToString("N") +
        ".tmp")))
...
try {
    ...
    & dotnet build (
        Join-Path $repositoryRoot (
            "src\GameAgent.Providers.Native\" +
            "GameAgent.Providers.Native.csproj")
    ) -c $Configuration
    if ($LASTEXITCODE -ne 0) {
        throw "GameAgent.Providers.Native build failed."
    }
    ...
    Copy-Item -LiteralPath (
        Join-Path $repositoryRoot (
            "src\GameAgent.Providers.Native\bin\$Configuration\" +
            "netstandard2.1\GameAgent.Providers.Native.dll")
    ) -Destination $libraryTarget
    Copy-Item -LiteralPath (
        Join-Path $repositoryRoot (
