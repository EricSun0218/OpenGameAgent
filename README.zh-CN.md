...
- 可选的 NPC 自进化：把结构化复盘和宿主可核验证据变成不可变版本行为 Skill，用当前可见的既有工具组合有序复合过程，并支持验证、评测降级与精确回滚；
- 可选的宿主发布通用行为目录：每个 NPC 必须显式采用并独立评测，不自动传播个人经验；
...
| 模块 | 能力 |
| --- | --- |
| Agent 内核 | 有界 ReAct 模型/工具循环、流式类型化消息、工具中间结果、执行中调整指令、追加输入、Hooks、取消、严格会话校验、安全的零工具模型会话压缩、提供方错误结果化 |
...
长会话可以组合 `SummarizingGameTranscriptCompactor` 与 `ModelGameTranscriptSummarizer`，使用任意 `IModelProvider` 生成可持久化摘要。官方适配器会先做 Provider 预检，并强制使用独立、零工具、无规范 session/actor 坐标的请求；隐藏推理、工具参数与 ID、附件字节、元数据和资源 URI 不会进入摘要 Prompt。消息数、输入字符、输出 Token、摘要字符、流事件、截止时间和重试次数均有独立上限，截断、工具调用、空结果、非法流、超时或超限结果都会明确失败并保留用量统计。
...
- [NPC 行为学习与自进化](docs/behavior-learning.zh-CN.md)
