...
- 有证据校验、不可变版本、宿主可选激活策略、评测降级和精确回滚的有界 NPC 行为学习；
...
| 官方扩展 | 工具策略、高风险执行批准与搜索、玩家结构化提问/推荐回复、目标、支持动态重规划与持久暂停/恢复且由宿主校验证据的有序任务清单、有界行为学习、记忆、产物、外部知识、带谱系与执行租约的重启可恢复委派、追踪和可持久并行工作流图 |
...
- [有界行为学习](docs/behavior-learning.zh-CN.md)
