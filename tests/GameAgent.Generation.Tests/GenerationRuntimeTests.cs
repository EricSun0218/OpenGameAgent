...
public sealed class GenerationRuntimeTests
{
    ...
    public async Task Submit_is_idempotent_for_canonically_equal_request()
    {
        ...
        var first = await runtime.SubmitAsync(Request("same", "{\"b\":2,\"a\":1}"));
        var second = await runtime.SubmitAsync(Request("same", "{\"a\":1,\"b\":2}"));
    ...
    public async Task Submit_rejects_operation_identity_reuse_with_changed_payload()
    {
        ...
        var runtime = Runtime(provider);
        await runtime.SubmitAsync(Request("same", "{\"value\":1}"));

        var exception = await Assert.ThrowsAsync<GenerationOperationException>(
            async () => await runtime.SubmitAsync(Request("same", "{\"value\":2}")));
    ...
    public async Task Submit_resumes_a_durably_queued_request_but_never_replays_uncertain_dispatch()
    {
        ...
        var completed = await runtime.SubmitAsync(request);
        ...
        var uncertain = await runtime.SubmitAsync(uncertainRequest);
    ...
    public async Task Shared_store_claims_provider_dispatch_once_across_runtime_instances()
    {
        ...
        _ = await Task.WhenAll(
            left.SubmitAsync(request).AsTask(),
            right.SubmitAsync(request).AsTask());
    ...
    public async Task Refresh_and_cancel_preserve_job_modality()
    {
        ...
        var runtime = Runtime(provider);
        await runtime.SubmitAsync(Request("poll", "{}", GenerationModalities.StructuredContent));

        var completed = await runtime.RefreshAsync("poll");
        ...
        await runtime.SubmitAsync(Request("cancel", "{}", GenerationModalities.Video));
        await runtime.RequestCancellationAsync("cancel");
    ...
    public async Task Failed_artifact_import_leaves_recoverable_materializing_checkpoint()
    {
        ...
        var exception = await Assert.ThrowsAsync<GenerationOperationException>(
            async () => await runtime.SubmitAsync(
                Request("artifact", "{}", GenerationModalities.Video)));
        var pending = await runtime.TryGetAsync("artifact");
        ...
        var recovered = await runtime.RefreshAsync("artifact");
    ...
    public async Task Materialization_recovers_without_a_provider_job_identity()
    {
        ...
        await Assert.ThrowsAsync<GenerationOperationException>(
            async () => await runtime.SubmitAsync(
                Request("sync-artifact", "{}", GenerationModalities.Speech)));

        var recovered = await runtime.RecoverUnfinishedAsync();
    ...
    public async Task Invalid_accepted_provider_contract_is_durable_unknown()
    {
        ...
        var exception = await Assert.ThrowsAsync<GenerationOperationException>(
            async () => await runtime.SubmitAsync(Request("invalid", "{}")));
        var durable = await runtime.TryGetAsync("invalid");
    ...
    public async Task Tool_bridge_exposes_valid_tools_and_returns_generation_receipt()
    {
        ...
        var receipt = await bridge.HandleAsync(request);
