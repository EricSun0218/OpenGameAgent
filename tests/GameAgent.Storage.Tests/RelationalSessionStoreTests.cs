...
public sealed class RelationalSessionStoreTests
{
    ...
    public async Task SqlitePersistsCursorAndIdempotentEventsAcrossRestart()
    {
        ...
        try
        {
            ...
            {
                var first = await owner.AppendAtomicAsync(Event("event-1", "run"), 0);
                Assert.Equal(0, first.Sequence);
                Assert.Equal(1, first.Revision);
            }
            await using (var reopened = new SqliteSessionStore(connection))
            {
                var duplicate = await reopened.AppendAtomicAsync(Event("event-1", "run"), 999);
                Assert.True(duplicate.WasDuplicate);
                var second = await reopened.AppendAtomicAsync(Event("event-2", "run"), 1);
                Assert.Equal(2, second.Revision);
                Assert.Equal(2, (await reopened.GetRunCursorAsync("run")).NextSequence);
                Assert.Equal(2, (await reopened.ReadRunAsync("run", default)).Count);
            }
        }
        finally
        {
    ...
    public async Task SqliteRejectsMixedDuplicateAndNewAtomicBatch()
    {
        ...
        try
        {
            ...
            await store.AppendAtomicAsync(Event("event-1", "run"));
            await Assert.ThrowsAsync<JournalEntryConflictException>(
                async () => await store.AppendAtomicBatchAsync(
                    new[] { Event("event-1", "run"), Event("event-2", "run") }));
            Assert.Single(await store.ReadRunAsync("run", default));
        }
        finally
        {
            Delete(path);
    ...
    public async Task SqliteMaintainsOperationLedgerAndReconciliation()
    {
        ...
        {
            await using var store = new SqliteSessionStore(ConnectionString(path));
            await store.AppendAtomicAsync(Started("run"), 0);
            await store.AppendAtomicAsync(ActionEvent("request", RuntimeEventKinds.ActionRequested, Request("operation")), 1);
            Assert.Single(await store.ReadPendingOperationsAsync("run"));

            var receiptEvent = ActionEvent(
                "receipt",
                RuntimeEventKinds.ActionReceived,
                Receipt("operation", ReceiptStatuses.Succeeded));
            var reconciled = await store.ReconcileReceiptAsync(receiptEvent, 2);
            Assert.Equal(ReceiptStatuses.Succeeded, reconciled.Operation.LatestReceipt!.Status);
            Assert.Empty(await store.ReadPendingOperationsAsync("run"));
        }
        finally
        {
            Delete(path);
    ...
    public async Task SqliteDeduplicatesEquivalentOperationsAcrossEventIds()
    {
        ...
        try
        {
            ...
            var firstRequest = await store.AppendAtomicAsync(
                ActionEvent("request-1", RuntimeEventKinds.ActionRequested, request), 1);
            var duplicateRequest = await store.AppendAtomicAsync(
                ActionEvent("request-2", RuntimeEventKinds.ActionRequested, request), 1);
            ...
            var firstReceipt = await store.ReconcileReceiptAsync(
                ActionEvent("receipt-1", RuntimeEventKinds.ActionReceived, receipt), 2);
            receipt.ReceivedAt = receipt.ReceivedAt.AddMinutes(1);
            var duplicateReceipt = await store.ReconcileReceiptAsync(
                ActionEvent("receipt-2", RuntimeEventKinds.ActionReceived, receipt), 2);
            ...
            Assert.Equal(3, (await store.GetRunCursorAsync("run")).Revision);
        }
        finally
        {
            Delete(path);
    ...
    public async Task SqliteCompareAndSwapRejectsStaleWriter()
    {
        ...
        try
        {
            ...
            await first.AppendAtomicAsync(Event("event-1", "run"), 0);

            var attempts = new[]
            {
                first.AppendAtomicAsync(Event("event-2a", "run"), 1).AsTask(),
                second.AppendAtomicAsync(Event("event-2b", "run"), 1).AsTask()
            };
            try { await Task.WhenAll(attempts); } catch { }
            Assert.Equal(1, attempts.Count(static task => task.Status == TaskStatus.RanToCompletion));
            Assert.Equal(2, (await first.GetRunCursorAsync("run")).Revision);
        }
        finally
        {
            Delete(path);
    ...
    public async Task PostgresContractWhenConfigured()
    {
        ...
        await using (var second = new PostgresSessionStore(connection, options))
        {
            ...
            await first.AppendAtomicAsync(Started("ledger-run"), 0);
            var request = Request("postgres-operation", "ledger-run");
            await first.AppendAtomicAsync(
                ActionEvent("postgres-request", RuntimeEventKinds.ActionRequested, request), 1);
            Assert.Single(await second.ReadPendingOperationsAsync("ledger-run"));
            var receipt = Receipt("postgres-operation", ReceiptStatuses.Succeeded);
            await second.ReconcileReceiptAsync(
                ActionEvent("postgres-receipt", RuntimeEventKinds.ActionReceived, receipt, "ledger-run"), 2);
        }
        await using var reopened = new PostgresSessionStore(connection, options);
        Assert.Equal(2, (await reopened.GetRunCursorAsync("run")).Revision);
        Assert.Empty(await reopened.ReadPendingOperationsAsync("ledger-run"));
        Assert.Equal(
            ReceiptStatuses.Succeeded,
            (await reopened.GetOperationAsync("postgres-operation"))!.LatestReceipt!.Status);
        ...
        try
        {
            ...
            Assert.Equal(3, (await reopened.GetRunCursorAsync("run")).Revision);
        }
        finally
        {
