...
public sealed class HierarchicalBudgetTests
{
    ...
    public async Task Scope_definition_cannot_inject_unreceipted_usage()
    {
        ...
        var exception = await Assert.ThrowsAsync<HierarchicalBudgetException>(
            async () => await ledger.DefineScopeAsync(scope, cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task Sibling_children_share_root_tree_budget()
    {
        ...
        await ledger.DefineScopeAsync(Scope(
            ...
            null,
            maxOutputTokens: 100), cancellationToken: TestContext.Current.CancellationToken);
        await ledger.DefineScopeAsync(Scope(
            "child-a",
        ...
        await ledger.ChargeAsync(Charge("charge-a", "child-a", outputTokens: 60), cancellationToken: TestContext.Current.CancellationToken);
        var exception = await Assert.ThrowsAsync<HierarchicalBudgetException>(
            async () => await ledger.ChargeAsync(
                Charge("charge-b", "child-b", outputTokens: 50), cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task Replayed_charge_is_idempotent_across_scope_reads()
    {
        ...
        await ledger.DefineScopeAsync(Scope(
            ...
            null,
            maxOutputTokens: 1_000), cancellationToken: TestContext.Current.CancellationToken);
        await ledger.DefineScopeAsync(Scope(
            "run",
            BudgetScopeKinds.Run,
            "world",
        ...
        await ledger.ChargeAsync(charge, cancellationToken: TestContext.Current.CancellationToken);
        await ledger.ChargeAsync(charge, cancellationToken: TestContext.Current.CancellationToken);
