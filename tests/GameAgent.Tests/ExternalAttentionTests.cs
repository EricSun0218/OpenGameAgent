...
public sealed class ExternalAttentionTests
{
    ...
    public async Task Concurrent_identical_requests_are_idempotent()
    {
        ...
        Assert.Single(await store.ListPendingAsync("world-1", 10, TestContext.Current.CancellationToken));
    }
    ...
    public async Task Resolution_is_exactly_once_and_bound_to_world_state()
    {
        var store = new InMemoryExternalAttentionStore();
        var coordinator = new ExternalAttentionCoordinator(store);
        var created = await coordinator.RequestAsync(Request(), cancellationToken: TestContext.Current.CancellationToken);
        var resolution = Resolution("answer-1", "yes");

        var resolved = await coordinator.ResolveAsync(
            "choice-1",
            resolution,
            created.Revision, cancellationToken: TestContext.Current.CancellationToken);
        ...
        var conflict = await Assert.ThrowsAsync<ExternalAttentionException>(
            async () => await coordinator.ResolveAsync(
                ...
                resolved.Revision, cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task Concurrent_identical_resolutions_and_closures_are_idempotent()
    {
        ...
        var created = await coordinator.RequestAsync(resolvedRequest, cancellationToken: TestContext.Current.CancellationToken);
        ...
        created = await coordinator.RequestAsync(closedRequest, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task Waiting_requests_do_not_consume_runtime_capacity()
    {
        ...
        for (var index = 0; index < 100; index++)
        {
            ...
            await coordinator.RequestAsync(request, cancellationToken: TestContext.Current.CancellationToken);
        }

        var pending = await store.ListPendingAsync("world-1", 128, TestContext.Current.CancellationToken);
