...
public sealed class GameTimeTriggerTests
{
        ...
        int expectedSkipped)
    {
        ...
        var admission = await coordinator.AdmitAsync(
            ...
            Occurrences(1, 2, 3), cancellationToken: TestContext.Current.CancellationToken);
        ...
        bool expectedCoalesced)
    {
        ...
        var first = await coordinator.AdmitAsync(definition, Occurrences(1), cancellationToken: TestContext.Current.CancellationToken);
        var second = await coordinator.AdmitAsync(definition, Occurrences(2), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task Replaying_same_occurrences_is_idempotent()
    {
        ...
        var first = await coordinator.AdmitAsync(definition, Occurrences(1, 2), cancellationToken: TestContext.Current.CancellationToken);
        var replay = await coordinator.AdmitAsync(definition, Occurrences(1, 2), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task Coalesce_never_mutates_input_already_consumed_by_a_running_launch()
    {
        ...
        var running = await coordinator.RecordLaunchStateAsync(
            ...
            first.Launches[0].Revision, cancellationToken: TestContext.Current.CancellationToken);

        var next = await coordinator.AdmitAsync(definition, Occurrences(2, 3), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task Concurrent_replay_creates_one_durable_launch()
    {
        ...
        var state = await store.TryGetAsync(admissions[0].State.StateKey, TestContext.Current.CancellationToken);
    ...
    public async Task State_keys_are_unambiguous_when_identifiers_contain_separators()
    {
        ...
        var leftAdmission = await coordinator.AdmitAsync(left, Occurrences(1), cancellationToken: TestContext.Current.CancellationToken);
        var rightAdmission = await coordinator.AdmitAsync(right, Occurrences(1), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task Retention_never_discards_non_terminal_launches()
    {
        ...
        var exception = await Assert.ThrowsAsync<GameTriggerException>(async () =>
            await coordinator.AdmitAsync(definition, Occurrences(1, 2), cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task Terminal_launch_state_is_idempotent_but_cannot_reopen()
    {
        ...
        var admission = await coordinator.AdmitAsync(
            ...
            Occurrences(1), cancellationToken: TestContext.Current.CancellationToken);
        ...
        var completed = await coordinator.RecordLaunchStateAsync(
            ...
            launch.Revision, cancellationToken: TestContext.Current.CancellationToken);
        ...
        var exception = await Assert.ThrowsAsync<GameTriggerException>(async () =>
            await coordinator.RecordLaunchStateAsync(
                ...
                launch.LaunchId,
                GameTriggerLaunchStates.Running,
                completed.Revision, cancellationToken: TestContext.Current.CancellationToken));
        ...
        var exception = await Assert.ThrowsAsync<GameTriggerException>(
            async () => await coordinator.AdmitAsync(
                Definition(
                    GameTriggerCatchUpPolicies.All,
                    GameTriggerOverlapPolicies.Queue),
                occurrences, cancellationToken: TestContext.Current.CancellationToken));
