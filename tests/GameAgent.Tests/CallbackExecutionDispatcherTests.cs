...
public sealed class CallbackExecutionDispatcherTests
{
    ...
    public async Task ShutdownCancellationWaitsForAdmissionAndIsNotDropped()
    {
        ...
        var held = await dispatcher.ReserveAsync(cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task AwaitRegistrationRemainsInsideProcessCapacity()
    {
        ...
        Assert.True(
            second.TryExecute(
                () => new ValueTask<int>(9),
                out var independent));
        Assert.Equal(9, await independent.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));

        source.Complete();
        Assert.Equal(7, await pending!.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task PendingCompletionsHaveAProcessWideBoundAndRecover()
    {
        ...
        Assert.Equal(7, await first.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));
        Assert.True(
            SpinWait.SpinUntil(
        ...
        Assert.True(
            secondDispatcher.TryExecute(
                () => new ValueTask<int>(9),
                out var recovered));
        Assert.Equal(9, await recovered.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));

        secondSource.Complete();
        Assert.Equal(7, await second.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));
        ...
        bool invokesBeforeThrow)
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(
            () => completion.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task SuppressedFlowCannotObserveAWorkersPriorRequestContext()
    {
        ...
        Assert.Equal(
            "first-request",
            await first.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));
        ...
        Assert.Null(await second!.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task NullContextWorkItemsCannotPolluteAReusableWorker()
    {
        ...
        {
            await workerPool.StopAsync().WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        }
    }
    ...
    public async Task AsyncResultUsesTheInvocationExecutionContext()
    {
        ...
        await sourceCompletion.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        Assert.Equal(
            "caller-context",
            await completion.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task BlockedDedicatedWorkerDoesNotStrandNextCallback()
    {
        ...
        try
        {
            ...
            Assert.Equal(
                2,
                await second.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));
        }
        finally
        {
            release.Set();
    ...
    public async Task StopDrainsQueuedWorkAndTerminatesEveryWorker()
    {
        ...
        try
        {
            ...
            release.Set();
            await Task.WhenAll(workers).WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
            ...
            Assert.Equal(42, await queued.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));
            await stopped.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        }
        finally
        {
            release.Set();
