...
public sealed class ProviderAttemptRunnerTests
{
    ...
    public async Task RequestPreparationValueTaskResultIsReisolated()
    {
        ...
        try
        {
            ...
            await provider.ResultEntered.WaitAsync(
                TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
            var error = await Assert.ThrowsAsync<ProviderException>(
                () => run.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));
            Assert.Equal("provider_request_preparation_timeout", error.Code);
        ...
        await Assert.Single(detached).WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        ...
        try
        {
            ...
            var cleanupFailure =
                await Assert.ThrowsAsync<ProviderException>(
                    () => cleanup.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task StreamStartupCapacityFailureIsKnownZeroAndRecovers()
    {
        ...
        finally
        {
            ...
            await Task.WhenAll(detached).WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        }
    ...
    public async Task PostStartMoveNextCapacityIsUnknownAndCleanupWaits()
    {
        ...
        try
        {
            await blockerEntered.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
            Assert.True(
                SpinWait.SpinUntil(
    ...
    public async Task PartialTimeoutWithoutUsageFailsClosedWithoutRetry()
    {
        ...
        Assert.False(error.Retryable);
        Assert.Equal(1, calls);
        await cancelled.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
    }
    ...
    public async Task QuarantinedProviderDoesNotBlockHealthyFallback()
    {
        ...
        var cancelledRun = runner.RunAsync(
                ...
                onDetachedCleanup: detached.Add)
            .AsTask();
        await primaryStarted.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        cancellation.Cancel();
        await Assert.ThrowsAnyAsync<OperationCanceledException>(
            () => cancelledRun);
        var cleanup = Assert.Single(detached);
    ...
    public async Task CooldownSkipsFailedRouteAcrossNewRunAndCapturedPlan()
    {
        ...
        var capturedRecoveryPlan = runner.CaptureRoutePlan(cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task CooldownExpiryAdmitsOnlyOneConcurrentHalfOpenProbe()
    {
        ...
        await probeEntered.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
