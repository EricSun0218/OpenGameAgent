...
public sealed class HeadlessAgentRuntimeTests
{
    ...
    public async Task TimedOutAppendCannotBeOvertakenByTerminalCheckpoint()
    {
        ...
        var run = runtime.RunAsync(request).AsTask();
        await store.Blocked.Task.WaitAsync(TimeSpan.FromSeconds(2));
        await Task.Delay(75);
    ...
    public async Task DetachedHostActionsRemainGloballyBoundedUntilTheyFinish()
    {
        ...
        var firstOutcome = await runtime.RunAsync(first)
            .AsTask()
            .WaitAsync(TimeSpan.FromSeconds(2));
        ...
        var secondOutcome = await runtime.RunAsync(second)
            .AsTask()
            .WaitAsync(TimeSpan.FromSeconds(2));
        ...
        host.Release.TrySetResult();
        await host.FirstCompleted.Task.WaitAsync(TimeSpan.FromSeconds(2));
        await WaitUntilAsync(
            () => runtime.InFlightActionCount == 0,
            TimeSpan.FromSeconds(2));
        ...
        var thirdOutcome = await runtime.RunAsync(third)
            .AsTask()
            .WaitAsync(TimeSpan.FromSeconds(2));
