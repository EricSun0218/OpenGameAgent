...
public sealed class RuntimeBuilderTests
{
    ...
    public async Task DirectDurableRunHidesToolsAndSkillsAndCapturesMode()
    {
        ...
        DurableExecutionPolicyBinding.Attach(
            ...
            built.Runtime.CaptureExecutionPolicyIdentity(
                DurableExecutionModes.Direct, cancellationToken: TestContext.Current.CancellationToken));

        var outcome = await built.Runtime.RunAsync(request, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task DirectDurableRunRejectsSkillActivationBeforeDispatch()
    {
        ...
        await Assert.ThrowsAsync<ArgumentException>(
            () => built.Runtime.RunAsync(request, cancellationToken: TestContext.Current.CancellationToken).AsTask());

        Assert.Equal(0, provider.CallCount);
        Assert.Empty(await store.ReadRunAsync(request.Run.RunId, TestContext.Current.CancellationToken));
    }
    ...
    public async Task BuilderUsesAndStopsCustomConversationContextEngine()
    {
        ...
        await built.StopAsync(cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task BuilderSurfacesIncompleteContextCleanupAndCanRetry()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(
            () => built.StopAsync(cancellationToken: TestContext.Current.CancellationToken).AsTask().WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));
        Assert.False(built.Runtime.ShutdownResourceCleanupCompleted);
        Assert.False(store.WasDisposed);

        await built.StopAsync(cancellationToken: TestContext.Current.CancellationToken).AsTask().WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        Assert.True(engine.CleanupCompleted);
    ...
    public async Task LifecycleStopTracksMiddlewareAlreadyInFlight()
    {
        ...
        await invocation.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
    }
    ...
    public async Task RunInputIsSnapshottedBeforeLifecycleMiddlewareAwaits()
    {
        ...
        try
        {
            ...
            var running = built.Runtime.RunAsync(request, cancellationToken: TestContext.Current.CancellationToken).AsTask();
            await middleware.Entered.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ResumeGuardIsSnapshottedBeforeLifecycleMiddlewareAwaits()
    {
        ...
        try
        {
            ...
            var resuming = built.Runtime.ResumeAsync(
                    initial.Run.RunId,
                    guard: guard, cancellationToken: TestContext.Current.CancellationToken)
                .AsTask();
            await middleware.Entered.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ResumeLifecycleReceivesRecoveredRunIdentityAndGameContext()
    {
        ...
        try
        {
            ...
            _ = await built.Runtime.ResumeAsync(initial.Run.RunId, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task RuntimePolicyReloadAppliesOnlyToTheNextLoopInvocation()
    {
        ...
        try
        {
            ...
            DurableExecutionPolicyBinding.Attach(
                firstRequest.Run,
                built.Runtime.CaptureExecutionPolicyIdentity(cancellationToken: TestContext.Current.CancellationToken));
            var first = await built.Runtime.RunAsync(firstRequest, cancellationToken: TestContext.Current.CancellationToken);
            ...
            DurableExecutionPolicyBinding.Attach(
                secondRequest.Run,
                built.Runtime.CaptureExecutionPolicyIdentity(cancellationToken: TestContext.Current.CancellationToken));
            var second = await built.Runtime.RunAsync(secondRequest, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ExecutionPolicyBindingRejectsBeforeProviderOrToolDispatch()
    {
        ...
        try
        {
            ...
            var retried = await built.Runtime.RunAsync(request, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ExecutionPolicyIdentityUsesEffectiveConfiguredModelId()
    {
        ...
        var firstIdentity =
            first.Runtime.CaptureExecutionPolicyIdentity(cancellationToken: TestContext.Current.CancellationToken);
        var secondIdentity =
            second.Runtime.CaptureExecutionPolicyIdentity(cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ResumeCapturesOnePolicyLeaseAfterOperationReconciliation()
    {
        ...
        try
        {
                         ...
                         .Build())
            {
                ...
                var waiting = await first.Runtime.RunAsync(request, cancellationToken: TestContext.Current.CancellationToken);
            ...
            var outcome = await resumed.Runtime.ResumeAsync(
                ...
                reconciler, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task TerminalResumeDoesNotRequireObsoleteExecutionPolicy()
    {
        ...
        try
        {
            ...
            DurableExecutionPolicyBinding.Attach(
                request.Run,
                built.Runtime.CaptureExecutionPolicyIdentity(cancellationToken: TestContext.Current.CancellationToken));
            var completed = await built.Runtime.RunAsync(request, cancellationToken: TestContext.Current.CancellationToken);
            skills.Replace(
                new[]
                {
            ...
            var replayed = await built.Runtime.ResumeAsync(
                request.Run.RunId, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ResumeReconcilesPendingOperationBeforePolicyMismatch()
    {
        ...
        try
        {
            ...
            DurableExecutionPolicyBinding.Attach(
                request.Run,
                originalPolicy);
            var waiting = await built.Runtime.RunAsync(request, cancellationToken: TestContext.Current.CancellationToken);
            Assert.Equal(RunStates.Reconciling, waiting.Run.State);
            skills.Replace(
                new[]
                {
    ...
    public async Task LocalSkillReloadPublishesIntoTheRunningBuilderRegistry()
    {
        ...
        {
            WriteSkill("first");
            var initial = packages.Reload(cancellationToken: TestContext.Current.CancellationToken);
            ...
            WriteSkill("second");
            var reloaded = packages.Reload(cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task BuilderCleanupNeverSynchronouslyWaitsForAsyncStore()
    {
        ...
        await store.DisposeEntered.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);

        Assert.False(cleanup.IsCompleted);
        Assert.False(store.WasDisposed);

        store.ReleaseDispose();
        await cleanup.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        Assert.True(store.WasDisposed);
    }
    ...
    public async Task BuilderPublishesDisposeTaskBeforeReentrantStoreCleanup()
    {
        ...
        await dispose.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task PreStoppedRuntimePublishesShutdownBeforeReentrantFlush()
    {
        ...
        await built.Runtime.WaitForShutdownDrainAsync(cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task CancelledShutdownWaitCanReplaySharedCleanupFailure()
    {
        ...
        await store.FlushEntered.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        cancellation.Cancel();
        await Assert.ThrowsAnyAsync<OperationCanceledException>(
            () => cancelledWait);
        ...
        var replayed = await Assert.ThrowsAsync<IOException>(
            () => built.StopAsync(cancellationToken: TestContext.Current.CancellationToken).AsTask());
    ...
    public async Task BuilderDisposeDrainsOwnedMemoryBeforeOtherDependencies()
    {
        ...
        var recall = memory.RecallAsync(
                new MemoryQuery(
                    ...
                    ProtocolJson.ParseElement("{}")), cancellationToken: TestContext.Current.CancellationToken)
            .AsTask();
        await provider.Started.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        var recallReport = await recall.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        ...
        try
        {
            await Task.Delay(TimeSpan.FromMilliseconds(120), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ShutdownCancelsAndDrainsActiveRunBeforeDisposingStore()
    {
        ...
        var startedOrCompleted = await Task.WhenAny(
            ...
            run,
            Task.Delay(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));
        ...
        var stop = built.StopAsync(cancellationToken: TestContext.Current.CancellationToken).AsTask();
        ...
        var outcome = await run.WaitAsync(TimeSpan.FromSeconds(5), cancellationToken: TestContext.Current.CancellationToken);
        await stop.WaitAsync(TimeSpan.FromSeconds(5), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ShutdownDrainsActiveChildBeforeDisposingStore()
    {
        ...
        provider.Release.TrySetResult();
        _ = await child.WaitAsync(TimeSpan.FromSeconds(5), cancellationToken: TestContext.Current.CancellationToken);
        await stop.WaitAsync(TimeSpan.FromSeconds(5), cancellationToken: TestContext.Current.CancellationToken);
        Assert.True(store.RunCancellationCommitted);
        Assert.False(store.DisposedBeforeRunCancellation);
        Assert.True(store.WasDisposed);
    ...
    public async Task ShutdownTimeoutDefersOwnedCleanupUntilActiveLeaseDrains()
    {
        ...
        _ = await run.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        await stop.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task DetachedProviderCleanupDelaysOwnedResourceDisposal()
    {
        ...
        var stop = built.StopAsync(cancellationToken: TestContext.Current.CancellationToken).AsTask();
        await provider.CallbackInvoked.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
                                 ...
                                 .Build())
            {
                ...
                var outcome = await coordinator.RunAsync(
                    new MultiActorDecisionBatch(
                        ...
                        new[] { request }), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task MultiActorAbandonDurablyCancelsAfterJournalReopen()
    {
        ...
        try
        {
            ...
            var durable = await replay.Runtime.ResumeAsync(
                participant.RunId, cancellationToken: TestContext.Current.CancellationToken);
