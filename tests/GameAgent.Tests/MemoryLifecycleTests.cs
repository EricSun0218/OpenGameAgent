...
public sealed class MemoryLifecycleTests
{
    ...
    public async Task LifecycleCombinesProvidersAndReportsPartialRecall()
    {
        ...
        var report = await lifecycle.RecallAsync(
            Query("castle", worldId: "world", requireCommitted: true), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task LifecycleEnforcesIsolationOnUnfilteredProviders()
    {
        ...
        var report = await lifecycle.RecallAsync(query, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task QueryTransformerCapacityFallsBackAndRecovers()
    {
        ...
        {
            await entered.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
            _ = await lifecycle.RecallAsync(Query("original"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ResultRerankerCapacityFallsBackAndRecovers()
    {
        ...
        try
        {
            var fallback = await lifecycle.RecallAsync(Query("original"), cancellationToken: TestContext.Current.CancellationToken);
        ...
        var recovered = await lifecycle.RecallAsync(Query("original"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task LifecycleFailClosesPerspectiveForUnfilteredProviders()
    {
        ...
        var defaultReport = await lifecycle.RecallAsync(QueryFor(), cancellationToken: TestContext.Current.CancellationToken);
        var observerReport = await lifecycle.RecallAsync(QueryFor(observer), cancellationToken: TestContext.Current.CancellationToken);
        var privilegedReport = await lifecycle.RecallAsync(
            QueryFor(includeAllPerspectives: true), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task OversizedProviderResultIsRejectedWithoutMaterialization()
    {
        ...
        var report = await lifecycle.RecallAsync(
            Query("fact", worldId: "world"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ProviderResultsAreSnapshottedByIndexWithoutEnumeration()
    {
        ...
        var report = await lifecycle.RecallAsync(
            Query(
                "fact",
                worldId: "world",
                requireCommitted: true), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ProviderIdentityIsFrozenAtRegistration()
    {
        ...
        var report = await lifecycle.RecallAsync(Query("fact"), cancellationToken: TestContext.Current.CancellationToken);
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(
            async () => await lifecycle.CommitAsync(
                Record(
                    "unsafe",
                    "rumor",
                    Provenance("world", 1, committed: false)), cancellationToken: TestContext.Current.CancellationToken));
        await lifecycle.CommitAsync(
            Record(
                "safe",
                "fact",
                Provenance("world", 1, committed: true)), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task PrefetchIsKeyedAndConsumedOnce()
    {
        ...
        var first = await lifecycle.TakePrefetchedAsync("turn-1", cancellationToken: TestContext.Current.CancellationToken);
        var second = await lifecycle.TakePrefetchedAsync("turn-1", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ConcurrentSameKeyPrefetchStartsOnlyOneProviderQuery()
    {
        ...
        await provider.Entered.WaitAsync(TimeSpan.FromSeconds(1), cancellationToken: TestContext.Current.CancellationToken);
        Assert.Equal(1, provider.Calls);

        provider.Release();
        Assert.NotNull(await lifecycle.TakePrefetchedAsync("shared", cancellationToken: TestContext.Current.CancellationToken));
    }
    ...
    public async Task ShutdownTracksAPrefetchAfterItIsConsumed()
    {
        ...
        var consumed = lifecycle
            .TakePrefetchedAsync("consumed", cancellationToken: TestContext.Current.CancellationToken)
            .AsTask();

        await lifecycle.DisposeAsync();
        provider.Release();

        var report = await consumed.WaitAsync(TimeSpan.FromSeconds(1), cancellationToken: TestContext.Current.CancellationToken);
        Assert.Empty(report!.Results);
    }
    ...
    public async Task SynchronousPrefetchProviderDoesNotHoldAdmissionLock()
    {
        ...
        await prefetch.WaitAsync(TimeSpan.FromSeconds(1), cancellationToken: TestContext.Current.CancellationToken);
        await lifecycle.DisposeAsync();
    }
    ...
    public async Task NonCooperativeProviderFailsSoftWithoutHidingHealthyMemory()
    {
        ...
        try
        {
            var report = await lifecycle
                .RecallAsync(Query("harbor", worldId: "world"), cancellationToken: TestContext.Current.CancellationToken)
                .AsTask()
                .WaitAsync(TimeSpan.FromSeconds(1), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ProviderTimeoutDispatchesCancellationAndReleasesSlot()
    {
        ...
        var report = await lifecycle.RecallAsync(
            Query("cancel-provider", worldId: "world"), cancellationToken: TestContext.Current.CancellationToken);

        Assert.True(report.IsPartial);
        Assert.Contains(provider.ProviderId, report.FailedProviderIds);
        await provider.Cancelled.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
    }
    ...
    public async Task DisposeWaitsForDetachedProviderCleanup()
    {
        ...
        {
            var report = await lifecycle.RecallAsync(
                Query("harbor", worldId: "world"), cancellationToken: TestContext.Current.CancellationToken);
            Assert.True(report.IsPartial);
            ...
            await dispose.WaitAsync(TimeSpan.FromSeconds(1), cancellationToken: TestContext.Current.CancellationToken);
            Assert.True(lifecycle.DetachedProviderCallsDrainedOnDispose);
        }
        finally
        {
    ...
    public async Task ShutdownDrainWaitsPastBoundedProviderTimeout()
    {
        ...
        try
        {
            ...
            drain = lifecycle.WaitForShutdownDrainAsync(cancellationToken: TestContext.Current.CancellationToken).AsTask();
            ...
            provider.Release();
            await drain.WaitAsync(TimeSpan.FromSeconds(1), cancellationToken: TestContext.Current.CancellationToken);
        ...
        finally
        {
            ...
            if (drain is null)
            {
                await lifecycle.WaitForShutdownDrainAsync(cancellationToken: TestContext.Current.CancellationToken);
            }
            else
            {
                await drain.WaitAsync(TimeSpan.FromSeconds(1), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ShutdownDrainCallerCancellationDoesNotWaitForBoundedDispose()
    {
        ...
        try
        {
            ...
            await Assert.ThrowsAnyAsync<OperationCanceledException>(
                () => cancelledWait.WaitAsync(
                    TimeSpan.FromMilliseconds(250), cancellationToken: TestContext.Current.CancellationToken));
        ...
        finally
        {
            ...
            if (cancelledWait is not null)
            {
                try
                {
                    await cancelledWait.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
                }
            ...
            await lifecycle
                .WaitForShutdownDrainAsync(cancellationToken: TestContext.Current.CancellationToken)
                .AsTask()
    ...
    public async Task ShutdownAdmissionFailureIsExplicitAndRetriable()
    {
        ...
        try
        {
            ...
            await lifecycle.RecallAsync(
                Query("still-open", worldId: "world"), cancellationToken: TestContext.Current.CancellationToken);
            occupied!.Dispose();
            occupied = null;
    ...
    public async Task QueryTransformersMayRewriteMeaningButCannotBroadenIsolation()
    {
        ...
        await lifecycle.RecallAsync(
            Query("original", worldId: "world-a"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task HostileRerankerCollectionFallsBackToOwnedSnapshot()
    {
        ...
        var report = await lifecycle.RecallAsync(Query("original"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task NonCooperativeQueryTransformerIsBoundedAndDrained()
    {
        ...
        try
        {
            var recall = lifecycle.RecallAsync(Query("original"), cancellationToken: TestContext.Current.CancellationToken).AsTask();
            await transformer.Entered.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);

            await recall.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
            Assert.Equal(
                "original",
                provider.Query!.Query.GetProperty("text").GetString());
    ...
    public async Task ProcessingTimeoutDispatchesCancellationAndReleasesSlot()
    {
        ...
        await lifecycle.RecallAsync(Query("original"), cancellationToken: TestContext.Current.CancellationToken);

        await transformer.Cancelled.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        Assert.Equal(
            "original",
            provider.Query!.Query.GetProperty("text").GetString());
    }
    ...
    public async Task DetachedQueryTransformerKeepsItsOriginalStageInput()
    {
        ...
        {
            var recall = lifecycle.RecallAsync(Query("original"), cancellationToken: TestContext.Current.CancellationToken).AsTask();
            await scheduler.FirstQueued.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
            await recall.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
            Assert.Equal(
                "later-stage",
                provider.Query!.Query.GetProperty("text").GetString());
    ...
    public async Task DetachedRerankerKeepsItsOriginalStageInput()
    {
        ...
        try
        {
            var recall = lifecycle.RecallAsync(Query("original"), cancellationToken: TestContext.Current.CancellationToken).AsTask();
            await scheduler.FirstQueued.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
            var report = await recall.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
            Assert.Equal(
