...
public sealed class BoundedConversationContextEngineTests
{
        ...
        try
        {
            ...
            await Assert.ThrowsAsync<TimeoutException>(
                () => wrapper.PrepareAsync(
                        ...
                        "capacity-turn",
                        messages,
                        messages.Select(message => message.MessageId).ToArray(), cancellationToken: TestContext.Current.CancellationToken)
                    .AsTask());
        }
        finally
        {
    ...
    public async Task CheckpointAndStopCapacityFailuresAreRetryable()
    {
        ...
        try
        {
            await checkpointEntered.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        ...
        finally
        {
            ...
            await checkpointBlocker.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        }
        ...
        try
        {
            await stopEntered.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        ...
        {
            stopRelease.Set();
            await stopBlocker.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        }
    ...
    public async Task OversizedCustomViewIsRejectedByRuntimeLimits()
    {
        ...
        var error = await Assert.ThrowsAsync<RuntimeContentLimitException>(
            () => wrapper.PrepareAsync(
                    ...
                    "oversized-turn",
                    Messages(),
                    stablePrefixMessageIds: null, cancellationToken: TestContext.Current.CancellationToken)
                .AsTask());
        ...
        foreach (var drop in new[] { false, true })
        {
            ...
            await Assert.ThrowsAsync<InvalidDataException>(
                () => wrapper.PrepareAsync(
                        ...
                        "stable-turn",
                        input,
                        input.Select(message => message.MessageId).ToArray(), cancellationToken: TestContext.Current.CancellationToken)
                    .AsTask());
            Assert.True(await wrapper.StopAsync());
        }
    }
    ...
    public async Task NonReturningInnerShutdownDoesNotBlockBoundedStop()
    {
        ...
        await stopEntered.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        Assert.False(await first.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));
