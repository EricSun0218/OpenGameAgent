...
public sealed class PersistentAgentGraphTests
{
    ...
    public async Task Bulk_registration_preserves_ten_thousand_durable_identities()
    {
        ...
        var registered = await graph.RegisterManyAsync(nodes, cancellationToken: TestContext.Current.CancellationToken);
        var state = await store.ReadAsync(TestContext.Current.CancellationToken);
    ...
    public async Task Coalescing_replaces_payload_but_keeps_original_message_identity()
    {
        ...
        await graph.RegisterAsync(Node("npc"), cancellationToken: TestContext.Current.CancellationToken);
        await graph.EnqueueAsync("npc", Message("first", "movement", 1, 1.25), cancellationToken: TestContext.Current.CancellationToken);
        await graph.EnqueueAsync("npc", Message("second", "movement", 2, 3.5), cancellationToken: TestContext.Current.CancellationToken);

        var node = await graph.TryGetAsync("npc", cancellationToken: TestContext.Current.CancellationToken);
        ...
        await graph.EnqueueAsync("npc", Message("second", "movement", 2, 3.5), cancellationToken: TestContext.Current.CancellationToken);
        await graph.EnqueueAsync("npc", Message("first", "movement", 1, 1.25), cancellationToken: TestContext.Current.CancellationToken);
        message = Assert.Single((await graph.TryGetAsync("npc", cancellationToken: TestContext.Current.CancellationToken))!.Mailbox);
        Assert.Equal(2, message.Revision);
        Assert.Single(message.AcceptedMessages);

        var conflict = await Assert.ThrowsAsync<PersistentAgentGraphException>(async () =>
            await graph.EnqueueAsync("npc", Message("second", "movement", 3, 9.5), cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task Delivery_ack_is_idempotent_after_an_unknown_commit_result()
    {
        ...
        var enqueued = await graph.EnqueueAsync(
            "npc",
            Message("message", null, 1, 1.25), cancellationToken: TestContext.Current.CancellationToken);
        var revision = Assert.Single(enqueued.Mailbox).Revision;

        var delivered = await graph.MarkDeliveredAsync("npc", "message", revision, cancellationToken: TestContext.Current.CancellationToken);
        var replay = await graph.MarkDeliveredAsync("npc", "message", revision, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task Concurrent_lease_touches_preserve_the_highest_durable_ordering_key()
    {
        ...
        var first = await residency.AcquireAsync("npc", 1, cancellationToken: TestContext.Current.CancellationToken);
        var second = await residency.AcquireAsync("npc", 2, cancellationToken: TestContext.Current.CancellationToken);
        first.Touch(100);

        await first.DisposeAsync();
        await second.DisposeAsync();

        Assert.Equal(100, (await graph.TryGetAsync("npc", cancellationToken: TestContext.Current.CancellationToken))!.LastAccessOrderingKey);
    ...
    {
        var graph = new PersistentAgentGraph(new InMemoryPersistentAgentGraphStore());
        await graph.RegisterManyAsync(new[] { Node("a"), Node("b") }, cancellationToken: TestContext.Current.CancellationToken);
        ...
        await Assert.ThrowsAsync<TimeoutException>(async () =>
            await residency.AcquireAsync("a", 1, cancellationToken: TestContext.Current.CancellationToken));
        ...
        await (await residency.AcquireAsync("b", 2, cancellationToken: TestContext.Current.CancellationToken)).DisposeAsync();
        await residency.DisposeAsync();
    }
    ...
    public async Task Agent_owning_unsettled_side_effect_cannot_be_evicted()
    {
        ...
        await graph.TransitionAsync(
            ...
            1,
            ownsUnsettledSideEffect: true, cancellationToken: TestContext.Current.CancellationToken);
        await graph.TransitionAsync(
            "npc",
        ...
        var exception = await Assert.ThrowsAsync<PersistentAgentGraphException>(
            async () => await graph.TransitionAsync(
                ...
                3,
                ownsUnsettledSideEffect: false, cancellationToken: TestContext.Current.CancellationToken));
    ...
    {
        var graph = new PersistentAgentGraph(new InMemoryPersistentAgentGraphStore());
        await graph.RegisterManyAsync(new[] { Node("a"), Node("b"), Node("c") }, cancellationToken: TestContext.Current.CancellationToken);
        await graph.EnqueueAsync("a", Message("mail-a", null, 1, 2.5), cancellationToken: TestContext.Current.CancellationToken);
        ...
        await (await residency.AcquireAsync("a", 1, cancellationToken: TestContext.Current.CancellationToken)).DisposeAsync();
        await (await residency.AcquireAsync("b", 2, cancellationToken: TestContext.Current.CancellationToken)).DisposeAsync();
        await (await residency.AcquireAsync("c", 3, cancellationToken: TestContext.Current.CancellationToken)).DisposeAsync();

        Assert.Equal(PersistentAgentStates.Evicted, (await graph.TryGetAsync("a", cancellationToken: TestContext.Current.CancellationToken))!.State);
        Assert.NotEqual(PersistentAgentStates.Evicted, (await graph.TryGetAsync("b", cancellationToken: TestContext.Current.CancellationToken))!.State);
        Assert.Single((await graph.TryGetAsync("a", cancellationToken: TestContext.Current.CancellationToken))!.Mailbox);
        Assert.Contains("a", loader.DisposedAgentIds);

        await (await residency.AcquireAsync("a", 4, cancellationToken: TestContext.Current.CancellationToken)).DisposeAsync();
        Assert.Single((await graph.TryGetAsync("a", cancellationToken: TestContext.Current.CancellationToken))!.Mailbox);
    ...
    public async Task Residency_refuses_capacity_when_all_idle_instances_own_side_effects()
    {
        ...
        var exception = await Assert.ThrowsAsync<PersistentAgentGraphException>(
            async () => await residency.AcquireAsync("b", 2, cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task Execution_and_model_capacity_are_independent_and_cancellable()
    {
        ...
        using var execution = await residency.AcquireExecutionAsync(cancellationToken: TestContext.Current.CancellationToken);
        using var model = await residency.AcquireModelCallAsync(cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task Agent_cannot_reload_while_its_previous_runtime_is_unloading()
    {
        ...
        var acquireB = residency.AcquireAsync("b", 2, cancellationToken: TestContext.Current.CancellationToken).AsTask();
        await loader.DisposeStarted.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        using var cancelled = new CancellationTokenSource(TimeSpan.FromMilliseconds(100));
    ...
    public async Task Slow_successful_unload_stays_quarantined_then_releases_capacity()
    {
        ...
        await Assert.ThrowsAsync<TimeoutException>(async () =>
            await residency.AcquireAsync("b", 2, cancellationToken: TestContext.Current.CancellationToken));
        Assert.Equal(1, residency.ResidentCount);
        Assert.Equal(1, loader.LoadCounts["a"]);
        ...
        Assert.Equal(0, residency.ResidentCount);
        await (await residency.AcquireAsync("b", 3, cancellationToken: TestContext.Current.CancellationToken)).DisposeAsync();
        await residency.DisposeAsync();
    }
