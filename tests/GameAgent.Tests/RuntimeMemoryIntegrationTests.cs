...
public sealed class RuntimeMemoryIntegrationTests
{
    ...
    public async Task BuilderRecallsUntrustedContextAndCommitsFinalTranscript()
    {
        ...
        try
        {
            ...
            var outcome = await built.Runtime.RunAsync(
                Request("memory-final-run"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task TerminalReceiptsDrivePolicyBeforeNextTurn()
    {
        ...
        try
        {
            ...
            var outcome = await built.Runtime.RunAsync(
                Request("memory-receipt-run"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task PreparedCommitReplaysWithoutReinvokingPolicy()
    {
        ...
        try
        {
            await using (var first = new GameAgentRuntimeBuilder(
                                 ...
                                 .Build())
            {
                var failed = await first.Runtime.RunAsync(Request(runId), cancellationToken: TestContext.Current.CancellationToken);
            ...
            await using (var recovered = new GameAgentRuntimeBuilder(
                                       ...
                                       .Build())
            {
                var outcome = await recovered.Runtime.ResumeAsync(runId, cancellationToken: TestContext.Current.CancellationToken);
            ...
            await using (var secondRecovery = new GameAgentRuntimeBuilder(
                                       ...
                                       .Build())
            {
                var outcome = await secondRecovery.Runtime.ResumeAsync(runId, cancellationToken: TestContext.Current.CancellationToken);
                Assert.Equal(RunStates.Failed, outcome.Run.State);
                Assert.Equal(2, store.ApplyCallCount);
    ...
    public async Task BuilderOwnsAndDrainsConfiguredMemoryLifecycle()
    {
        ...
        try
        {
            _ = await built.Runtime.RunAsync(Request("owned-memory-run"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task EmptyMutationDecisionUsesOneDurableSettlement()
    {
        ...
        try
        {
            ...
            var outcome = await built.Runtime.RunAsync(
                Request("empty-memory-run"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task SettledDecisionIsNotReinvokedAfterCrashWindow()
    {
        ...
        try
        {
            ...
            await using (var first = new GameAgentRuntimeBuilder(
                                 ...
                                 .Build())
            {
                var outcome = await first.Runtime.RunAsync(Request(runId), cancellationToken: TestContext.Current.CancellationToken);
                Assert.Equal(RunStates.Completed, outcome.Run.State);
            ...
            var resumed = await recovered.Runtime.ResumeAsync(runId, cancellationToken: TestContext.Current.CancellationToken);
        ...
        string deleteKind)
    {
        ...
        try
        {
            ...
            var outcome = await built.Runtime.RunAsync(
                Request("memory-delete-guard-" + deleteKind), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task RuntimeManagedUpsertCannotReplaceAnotherWorldRecord()
    {
        ...
        try
        {
            ...
            var outcome = await built.Runtime.RunAsync(
                Request("memory-upsert-world-collision"), cancellationToken: TestContext.Current.CancellationToken);
