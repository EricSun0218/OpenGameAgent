...
public sealed class ToolSchedulingTests
{
    ...
    public async Task SchedulerAllowsUnorderedCompletionButReturnsModelCallOrder()
    {
        ...
        var results = await scheduler.ExecuteAsync(plan, executor, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task WorldCommandIsAGlobalBarrierAcrossConcurrentBatches()
    {
        ...
        var readTask = scheduler.ExecuteAsync(readPlan, executor, cancellationToken: TestContext.Current.CancellationToken).AsTask();
        await executor.ReadEntered.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        var worldTask = scheduler.ExecuteAsync(worldPlan, executor, cancellationToken: TestContext.Current.CancellationToken).AsTask();
    ...
    public async Task SameAgentLocalWritesAreSerializedAcrossConcurrentBatches()
    {
        ...
        await Task.WhenAll(
            scheduler.ExecuteAsync(firstPlan, executor, cancellationToken: TestContext.Current.CancellationToken).AsTask(),
            scheduler.ExecuteAsync(secondPlan, executor, cancellationToken: TestContext.Current.CancellationToken).AsTask());
    ...
    public async Task QueueAndJsonLimitsFailClosed()
    {
        ...
        var firstTask = scheduler.ExecuteAsync(firstPlan, blockingExecutor, cancellationToken: TestContext.Current.CancellationToken).AsTask();
        await blockingExecutor.Entered.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        var queueException = await Assert.ThrowsAsync<ToolQueueCapacityExceededException>(
            () => scheduler.ExecuteAsync(secondPlan, blockingExecutor, cancellationToken: TestContext.Current.CancellationToken).AsTask());
    ...
    public async Task OversizedToolResultBecomesABoundedFailure()
    {
        ...
        var results = await scheduler.ExecuteAsync(plan, new DeepResultExecutor(), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ExpiredAbsoluteDeadlineNeverDispatchesTheExecutor()
    {
        ...
        var result = Assert.Single(
            await scheduler.ExecuteAsync(
                ...
                executor,
                clock, cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task DispatchGateRejectsBeforeExecutorWithoutAmbiguousEffects()
    {
        ...
        var result = Assert.Single(
            await scheduler.ExecuteReservedAsync(
                ...
                reservation,
                tryAcquireDispatchPermit: () => false, cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task SideEffectTimeoutFailsFastUntilExecutorStops()
    {
        ...
        var result = Assert.Single(
            await scheduler.ExecuteAsync(plan, executor, cancellationToken: TestContext.Current.CancellationToken).AsTask()
        ...
        var conflictingResult = Assert.Single(
            await scheduler
                .ExecuteAsync(conflictingPlan, executor, cancellationToken: TestContext.Current.CancellationToken)
                .AsTask()
                .WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));
        Assert.Equal(
        ...
        await executor.Completed.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        Assert.True(
            await scheduler.DrainDetachedExecutionsAsync(
                TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));
        ...
        var afterRelease = Assert.Single(
            await scheduler.ExecuteAsync(
                ...
                executor, cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task ThrowingCancellationCallbackCannotReleaseTimeoutLease()
    {
        ...
        var first = Assert.Single(
            await scheduler.ExecuteAsync(
                ...
                .AsTask()
                .WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));
        await executor.CallbackInvoked.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task HighFrequencyFastToolsCancelEveryTimeoutWait()
    {
        ...
        var results = await scheduler.ExecuteAsync(
            plan,
            new ImmediateExecutor(), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task TimeoutWaitCancellationCallbackCannotChangeToolResult()
    {
        ...
        var result = Assert.Single(
            await scheduler.ExecuteAsync(plan, new ImmediateExecutor(), cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task BlockingTimeoutCleanupExhaustsBoundedCapacityBeforeDispatch()
    {
        ...
        await delay.CallbackInvoked.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task DetachedCensusIsBoundedImmutableAndDrainsAfterAllExecutionsFinish()
    {
        ...
        Assert.False(
            await scheduler.DrainDetachedExecutionsAsync(
                TimeSpan.FromMilliseconds(10), cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task CallerCancellationRegistersDetachedExecutionAndDrainHonorsCancellation()
    {
        ...
        await executor.Entered.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        callerCancellation.Cancel();
        await Assert.ThrowsAnyAsync<OperationCanceledException>(
            () => execution.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));
        Assert.Equal(1, scheduler.DetachedExecutionCount);
        Assert.Equal(
            "caller_cancelled",
        ...
        await Assert.ThrowsAsync<ArgumentOutOfRangeException>(
            () => scheduler
                .DrainDetachedExecutionsAsync(Timeout.InfiniteTimeSpan, cancellationToken: TestContext.Current.CancellationToken)
                .AsTask());
    ...
    public async Task UnknownSideEffectStopsTheRemainingBatch()
    {
        ...
        var results = await scheduler
            .ExecuteAsync(plan, executor, cancellationToken: TestContext.Current.CancellationToken)
            .AsTask()
            .WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
