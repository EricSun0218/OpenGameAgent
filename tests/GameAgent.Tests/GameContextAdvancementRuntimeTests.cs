...
public sealed class GameContextAdvancementRuntimeTests
{
    ...
    public async Task RecoveryAfterAdvancementDoesNotReplayHostAction()
    {
        ...
        try
        {
            ...
            firstRun = first.Runtime.RunAsync(request, cancellationToken: TestContext.Current.CancellationToken).AsTask();
        ...
        finally
        {
            ...
            if (firstRun is not null)
            {
                try
                {
                    _ = await firstRun.WaitAsync(TimeSpan.FromSeconds(10), cancellationToken: TestContext.Current.CancellationToken);
                }
                catch
                {
    ...
    public async Task UnknownReceiptAdvancesOnlyAfterTerminalReconciliation()
    {
        ...
        try
        {
            ...
            var pending = await built.Runtime.RunAsync(request, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task AdvancementPrecedesMemoryPolicyAndNextRecall()
    {
        ...
        try
        {
            ...
            var outcome = await built.Runtime.RunAsync(
                RunRequest("coordinate-memory"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ConcurrentNpcAdvancementKeepsIncarnationsIsolated()
    {
        ...
        try
        {
            ...
            var outcomes = await Task.WhenAll(
                built.Runtime.RunAsync(first, cancellationToken: TestContext.Current.CancellationToken).AsTask(),
                built.Runtime.RunAsync(second, cancellationToken: TestContext.Current.CancellationToken).AsTask());
        ...
        string mutation)
    {
        ...
        try
        {
            ...
            {
                var outcome = await built.Runtime.RunAsync(
                    RunRequest("coordinate-tamper-" + mutation), cancellationToken: TestContext.Current.CancellationToken);
                Assert.Equal(RunStates.Completed, outcome.Run.State);
    ...
    public async Task InvalidLiveTransitionReturnsStableGameContextFailure()
    {
        ...
        try
        {
            ...
            var outcome = await built.Runtime.RunAsync(
                RunRequest("coordinate-live-invalid"), cancellationToken: TestContext.Current.CancellationToken);
