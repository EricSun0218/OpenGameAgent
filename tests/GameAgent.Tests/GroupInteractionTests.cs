...
public sealed class GroupInteractionTests
{
    ...
    public async Task StructuredPayloadIsSharedWithoutAssumingNaturalLanguage()
    {
        ...
        var aliceView = await store.ProjectAsync("session-1", alice, cancellationToken: TestContext.Current.CancellationToken);
        var bobView = await store.ProjectAsync("session-1", bob, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ExplicitAudienceAndIncarnationPreventPrivateLeakage()
    {
        ...
        Assert.Single(
            (await store.ProjectAsync("session", oldBob, cancellationToken: TestContext.Current.CancellationToken))!.Messages);
        Assert.Empty(
            (await store.ProjectAsync("session", carol, cancellationToken: TestContext.Current.CancellationToken))!.Messages);
        ...
        var newBobView = await store.ProjectAsync(
            "session",
            Identity("bob", 3), cancellationToken: TestContext.Current.CancellationToken);
        Assert.Equal(GroupInteractionWriteStatuses.Applied, replaced.Status);
        Assert.Empty(newBobView!.Messages);
        await Assert.ThrowsAsync<InvalidOperationException>(
            async () => await store.ProjectAsync("session", oldBob, cancellationToken: TestContext.Current.CancellationToken));
    }
    ...
    public async Task RetryAfterLaterWritesIsIdempotentAndDoesNotAppendAgain()
    {
        ...
        var first = await store.AppendAsync(request, cancellationToken: TestContext.Current.CancellationToken);
        var second = await store.AppendAsync(
            Append("append-two", 1, 0, alice, "message-two"), cancellationToken: TestContext.Current.CancellationToken);
        var replay = await store.AppendAsync(request, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ReusingOperationIdForDifferentPayloadFailsClosed()
    {
        ...
        await store.AppendAsync(
            Append("operation", 0, 0, alice, "message-a"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ConcurrentCompareAndSwapAllowsOnlyOneAppend()
    {
        ...
        var firstTask = store.AppendAsync(
                Append("first", 0, 0, alice, "message-first"), cancellationToken: TestContext.Current.CancellationToken)
            .AsTask();
        var secondTask = store.AppendAsync(
                Append("second", 0, 0, alice, "message-second"), cancellationToken: TestContext.Current.CancellationToken)
            .AsTask();
        ...
        Assert.Single((await store.ReadAsync("session", cancellationToken: TestContext.Current.CancellationToken))!.Messages);
    }
    ...
    public async Task ClosingSessionRejectsNewWritesButRetryStillResolves()
    {
        ...
        var closeRequest = new GroupInteractionCloseRequest(
            ...
            0,
            0);
        var closed = await store.CloseAsync(closeRequest, cancellationToken: TestContext.Current.CancellationToken);
        var rejected = await store.AppendAsync(
            Append("late", 1, 0, alice), cancellationToken: TestContext.Current.CancellationToken);
        var replay = await store.CloseAsync(closeRequest, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task AuthorAndAudienceMustMatchExactCurrentMembership()
    {
        ...
        await Assert.ThrowsAsync<ArgumentException>(
            async () => await store.AppendAsync(
                Append(
                    ...
                    0,
                    0,
                    Identity("alice", 1)), cancellationToken: TestContext.Current.CancellationToken));
        await Assert.ThrowsAsync<ArgumentException>(
            async () => await store.AppendAsync(
                new GroupInteractionAppendRequest(
                    "unknown-audience",
        ...
        Assert.Equal(0, (await store.ReadAsync("session", cancellationToken: TestContext.Current.CancellationToken))!.Revision);
    }
    ...
    public async Task ConfiguredLargeStructuredValueHasNoHiddenDigestLimit()
    {
        ...
        var created = await store.CreateAsync(
            new GroupInteractionCreateRequest(
                ...
                "group",
                scope,
                new[] { new GroupInteractionMember(alice) }), cancellationToken: TestContext.Current.CancellationToken);
        var appended = await store.AppendAsync(
            new GroupInteractionAppendRequest(
                "append-large",
                "large-session",
    ...
    public async Task OperationCapacityAlwaysReservesAClosingTransition()
    {
        ...
        await CreateAsync(store, alice);
        var appended = await store.AppendAsync(
            Append("append-before-reserve", 0, 0, alice), cancellationToken: TestContext.Current.CancellationToken);
        var full = await store.AppendAsync(
            Append(
                ...
                0,
                alice,
                "message-two"), cancellationToken: TestContext.Current.CancellationToken);
        var closed = await store.CloseAsync(
            new GroupInteractionCloseRequest(
                "close-reserved",
                "session",
                expectedRevision: 1,
                expectedMembershipRevision: 0), cancellationToken: TestContext.Current.CancellationToken);
