...
public sealed class MultiActorCoordinationTests
{
    ...
    public async Task BatchBoundsConcurrencyAndReturnsInputOrder()
    {
        ...
        var outcome = await coordinator.RunAsync(
            new MultiActorDecisionBatch(
                "tick-12",
                Coordinate(),
                sourceRuns), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task MaxConcurrentRunsIsCoordinatorWideAcrossBatches()
    {
        ...
        var first = coordinator.RunAsync(
                new MultiActorDecisionBatch(
                    ...
                    Coordinate(),
                    new[] { Request(1) }), cancellationToken: TestContext.Current.CancellationToken)
            .AsTask();
        var second = coordinator.RunAsync(
                new MultiActorDecisionBatch(
                    "coordinator-wide-second",
                    Coordinate(),
                    new[] { Request(2) }), cancellationToken: TestContext.Current.CancellationToken)
            .AsTask();

        await lifecycle.AllStarted.WaitAsync(TimeSpan.FromSeconds(5), cancellationToken: TestContext.Current.CancellationToken);
        await runtime.FirstEntered.WaitAsync(TimeSpan.FromSeconds(5), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ConcurrentRetryDoesNotAbortActivePreparedBatch()
    {
        ...
        var prepared = coordinator.PrepareBatch(
            new MultiActorDecisionBatch(
                ...
                Coordinate(),
                new[] { Request(1) }), cancellationToken: TestContext.Current.CancellationToken);
        var first = coordinator.RunPreparedBatchAsync(prepared, cancellationToken: TestContext.Current.CancellationToken).AsTask();
        await runtime.FirstEntered.WaitAsync(TimeSpan.FromSeconds(5), cancellationToken: TestContext.Current.CancellationToken);
        ...
        try
        {
            var error = await Assert.ThrowsAsync<
                ...
                () => coordinator.RunPreparedBatchAsync(prepared, cancellationToken: TestContext.Current.CancellationToken)
                    .AsTask()
    ...
    public async Task SharedRuntimeFencesBatchAcrossCoordinators()
    {
        ...
        var first = firstCoordinator
            .RunPreparedBatchAsync(firstPrepared, cancellationToken: TestContext.Current.CancellationToken)
            .AsTask();
        await runtime.FirstEntered.WaitAsync(TimeSpan.FromSeconds(5), cancellationToken: TestContext.Current.CancellationToken);
        ...
        try
        {
            var error = await Assert.ThrowsAsync<
                MultiActorBatchAlreadyActiveException>(
                () => secondCoordinator
                    .RunPreparedBatchAsync(secondPrepared, cancellationToken: TestContext.Current.CancellationToken)
                    .AsTask());
    ...
    public async Task QueuedParticipantCapacityRejectsBeforeLifecycle()
    {
        ...
        var first = coordinator.RunAsync(
                new MultiActorDecisionBatch(
                    ...
                    Coordinate(),
                    new[] { Request(1), Request(2) }), cancellationToken: TestContext.Current.CancellationToken)
            .AsTask();
        await runtime.FirstEntered.WaitAsync(TimeSpan.FromSeconds(5), cancellationToken: TestContext.Current.CancellationToken);
            ...
            var error = await Assert.ThrowsAsync<
                ...
                () => coordinator.RunAsync(
                        new MultiActorDecisionBatch(
                            ...
                            Coordinate(),
                            new[] { Request(3) }), cancellationToken: TestContext.Current.CancellationToken)
                    .AsTask());
    ...
    public async Task BatchPreservesEachActorsObserverIncarnation()
    {
        ...
        await coordinator.RunAsync(
            new MultiActorDecisionBatch(
                "observer-batch",
                Coordinate(),
                requests), cancellationToken: TestContext.Current.CancellationToken);
        ...
        await Assert.ThrowsAsync<ArgumentException>(
            () => coordinator.RunAsync(
                    new MultiActorDecisionBatch(
                        ...
                        new[] { Request(1, "session-b") }), cancellationToken: TestContext.Current.CancellationToken)
                .AsTask());
        ...
        await Assert.ThrowsAsync<ArgumentException>(
            () => coordinator.RunAsync(
                    new MultiActorDecisionBatch(
                        "contradictory-batch",
                        Coordinate(),
                        new[] { contradictory }), cancellationToken: TestContext.Current.CancellationToken)
                .AsTask());
            ...
            () => coordinator.RunAsync(
                    new MultiActorDecisionBatch(
                        ...
                        new MultiActorBatchBudget(
                            ...
                            maxActions,
                            maxDurationMs,
                            maxCostUsd)), cancellationToken: TestContext.Current.CancellationToken)
                .AsTask());
    ...
    public async Task AggregateRunBudgetExactReservationIsManifested()
    {
        ...
        var outcome = await coordinator.RunAsync(
            new MultiActorDecisionBatch(
                ...
                Coordinate(),
                new[] { Request(1), Request(2) },
                limit), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task OneActorFailureDoesNotCancelOtherActors()
    {
        ...
        var outcome = await coordinator.RunAsync(
            new MultiActorDecisionBatch(
                "tick-12",
                Coordinate(),
                Enumerable.Range(0, 4).Select(Request)), cancellationToken: TestContext.Current.CancellationToken);
        ...
        await Assert.ThrowsAsync<ArgumentException>(
            () => coordinator.RunAsync(
                    new MultiActorDecisionBatch(
                        "batch",
                        Coordinate(),
                        duplicateActor), cancellationToken: TestContext.Current.CancellationToken)
                .AsTask());
        ...
        await Assert.ThrowsAsync<ArgumentException>(
            () => coordinator.RunAsync(
                    new MultiActorDecisionBatch(
                        "batch",
                        Coordinate(),
                        duplicateDecision), cancellationToken: TestContext.Current.CancellationToken)
                .AsTask());
    }
    ...
    public async Task PausedActorResumesAfterCoordinatorRecreation()
    {
        ...
        var resumed = await recoveredCoordinator.ResumeParticipantAsync(
            "paused-batch",
            Assert.Single(batch.Manifest.Participants), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ParticipantResumeMergesCallerSemanticExpectation()
    {
        ...
        var result = await coordinator.ResumeParticipantAsync(
            ...
            expectation, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ParticipantResumeAdmissionIsBoundedAndReleased()
    {
        ...
        var first = coordinator.ResumeParticipantAsync(
                "bounded-resumes",
                batch.Manifest.Participants[0], cancellationToken: TestContext.Current.CancellationToken)
            .AsTask();
        await runtime.ResumeEntered.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        ...
        var capacity = await Assert.ThrowsAsync<
            ...
            () => coordinator.ResumeParticipantAsync(
                    "bounded-resumes",
                    batch.Manifest.Participants[1], cancellationToken: TestContext.Current.CancellationToken)
                .AsTask());
        ...
        Assert.True(
            (await coordinator.ResumeParticipantAsync(
                "bounded-resumes",
                batch.Manifest.Participants[1], cancellationToken: TestContext.Current.CancellationToken)).Outcome!.IsTerminal);
        Assert.Equal(0, coordinator.ActiveParticipantOperationCount);
    }
    ...
    public async Task ResumeFailureLeavesParticipantRetriable()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(
            () => coordinator.ResumeParticipantAsync(
                    "retry-resume",
                    participant, cancellationToken: TestContext.Current.CancellationToken)
                .AsTask());
        Assert.Empty(lifecycle.FinishedAgentIds);
        Assert.Equal(0, coordinator.ActiveParticipantOperationCount);
    ...
    public async Task AbandonedParticipantClosesHostLifecycleWindow()
    {
        ...
        var abandoned =
            await coordinator.ReconcileAbandonedParticipantAsync(
                "abandoned-participant",
                participant,
                "game_removed_actor", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task LateBatchStartIsNotRacedByAbort()
    {
        ...
        lifecycle.Release();
        await lifecycle.Landed.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        Assert.Equal(0, lifecycle.AbortCalls);
    }
    ...
    public async Task AbortCallbackCapacityFailureIsUncertainAndRecovers()
    {
        ...
        try
        {
            first = coordinator.RunAsync(
                ...
                .AsTask();
            await runtime.FirstEntered.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
            firstCancellation.Cancel();

            var aggregate = await Assert.ThrowsAsync<AggregateException>(
                () => first.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));
            ...
            await runtime.StartedBlocker!.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
            ...
            second = coordinator.RunAsync(
                ...
                .AsTask();
            await runtime.SecondEntered.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
            secondCancellation.Cancel();

            await Assert.ThrowsAnyAsync<OperationCanceledException>(
                () => second.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));
            Assert.Equal(
                "abort-capacity-recovered",
        ...
        finally
        {
            ...
            foreach (var pending in new[] { first, second })
            {
                ...
                try
                {
                    await pending.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
                }
    ...
    public async Task PreparedBatchIsADeepImmutableRequestSnapshot()
    {
        ...
        var outcome = await coordinator.RunPreparedBatchAsync(prepared, cancellationToken: TestContext.Current.CancellationToken);
        ...
        var error = Assert.Throws<RuntimeContentLimitException>(
            () => coordinator.PrepareBatch(
                new MultiActorDecisionBatch(
                    ...
                    Coordinate(),
                    new[] { request },
                    budget), cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task PreparedResumeRejectsOversizedContinuationLaneBeforeStart()
    {
        ...
        await Assert.ThrowsAsync<RuntimeContentLimitException>(
            () => coordinator.ResumeOrStartPreparedBatchAsync(
                    prepared,
                    continuations, cancellationToken: TestContext.Current.CancellationToken)
                .AsTask());
    ...
    public async Task PreparedResumeRequiresGuardedRuntimeBeforeLifecycle()
    {
        ...
        var error = await Assert.ThrowsAsync<DurableRunResumeGuardException>(
            () => coordinator.ResumeOrStartPreparedBatchAsync(prepared, cancellationToken: TestContext.Current.CancellationToken)
                .AsTask());
    ...
    public async Task MissingRunRejectsChangedDurableParticipantIdentity()
    {
        ...
        var outcome = await coordinator.ResumeOrStartPreparedBatchAsync(
            prepared, cancellationToken: TestContext.Current.CancellationToken);
        ...
        bool requestCancellation)
    {
        ...
        var outcome = await coordinator.ResumeOrStartPreparedBatchAsync(
            prepared,
            continuations,
            startMissing: startMissing, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task LatePreparedResumeFinishIsNotRacedByAbort()
    {
        ...
        Assert.Equal(0, lifecycle.AbortCalls);
        lifecycle.Release();
        await lifecycle.Landed.WaitAsync(TimeSpan.FromSeconds(5), cancellationToken: TestContext.Current.CancellationToken);
        Assert.Equal(0, lifecycle.AbortCalls);
    }
    ...
    public async Task PreparedResumeRejectsLyingExpectationEnumeration()
    {
        ...
        await Assert.ThrowsAsync<ArgumentException>(
            () => coordinator.ResumeOrStartPreparedBatchAsync(
                    prepared,
                    semanticExpectations: expectations, cancellationToken: TestContext.Current.CancellationToken)
                .AsTask());
