...
public sealed class OpenAiCompatibleProviderTests
{
        ...
        bool expectedReasoning)
    {
        ...
        _ = await CollectAsync(provider.StreamAsync(request, TestContext.Current.CancellationToken));
    ...
    public async Task UnsupportedPerOperationControlFailsBeforeTransport()
    {
        ...
        var error = await Assert.ThrowsAsync<ProviderException>(
            () => CollectAsync(provider.StreamAsync(request, TestContext.Current.CancellationToken)));
    ...
    public async Task RequestSnapshotSurvivesMutationDuringCredentialLookup()
    {
        ...
        var collection = CollectAsync(
            provider.StreamAsync(request, CancellationToken.None));
        await credentials.Started.WaitAsync(TimeSpan.FromSeconds(5), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task SecureTransportDoesNotForwardRedirectedRequest()
    {
        ...
        try
        {
            ...
            await redirectTask.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
            await Task.Delay(TimeSpan.FromMilliseconds(200), cancellationToken: TestContext.Current.CancellationToken);
