...
public sealed class RuntimeEventBusTests
{
    ...
    public async Task BufferedPublisherIsolatesABlockingObserver()
    {
        ...
        await observer.Started.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
    ...
    [Fact]
    public async Task BufferedPublishersShareBoundedObserverWorkerCapacity()
    {
        var dispatcher = new BoundedObserverWorkerDispatcher(capacity: 1);
        var blockingObserver = new BlockingPublisher();
        var countingObserver = new CountingPublisher();
        var first = new BufferedRuntimeEventPublisher(
            blockingObserver,
            capacity: 1,
            dispatcher);
        using var second = new BufferedRuntimeEventPublisher(
            countingObserver,
            capacity: 1,
            dispatcher);
        try
        {
            first.Publish(Event("blocking", EventDurabilities.Durable));
            await blockingObserver.Started.Task.WaitAsync(
            ...
            second.Publish(Event("rejected", EventDurabilities.Durable));

            Assert.Equal(1, second.WorkerRejections);
            Assert.Equal(1, second.DroppedDurableNotifications);
            Assert.Equal(0, countingObserver.CallCount);
            Assert.Equal(1, dispatcher.ActiveReservations);
            ...
            second.Publish(Event("accepted", EventDurabilities.Durable));
            await countingObserver.Published.Task.WaitAsync(
                TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);

            Assert.Equal(1, countingObserver.CallCount);
            Assert.True(
                await WaitUntilAsync(
                    () => dispatcher.ActiveReservations == 0,
                    TimeSpan.FromSeconds(2)));
    ...
    private sealed class CountingPublisher : IRuntimeEventPublisher
    {
        private int _callCount;

        public int CallCount => Volatile.Read(ref _callCount);

        public TaskCompletionSource<bool> Published { get; } =
            new(TaskCreationOptions.RunContinuationsAsynchronously);
