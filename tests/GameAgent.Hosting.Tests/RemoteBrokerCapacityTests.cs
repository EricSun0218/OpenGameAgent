...
public sealed class RemoteBrokerCapacityTests
{
    ...
    public async Task PendingActionLimitCannotBeOversubscribed()
    {
        ...
        var first = channel.SubmitAsync(Request("first"), cancellationToken: TestContext.Current.CancellationToken).AsTask();
        await socket.Sent.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);

        var error = await Assert.ThrowsAsync<TenantCapacityExceededException>(
            async () => await channel.SubmitAsync(Request("second"), cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task ReceiptForAnotherOperationIsRejected()
    {
        ...
        await Assert.ThrowsAsync<InvalidDataException>(
            async () => await channel.SubmitAsync(Request("expected"), cancellationToken: TestContext.Current.CancellationToken));
