    ...
    public async Task CompletedAgentRunUsesStableStageIdentity()
    {
        ...
        var result = await runner.ExecuteAsync(
            ...
            new WorkflowRunRequest(
                "completed",
                "owner",
                WorkflowTestData.Json("""{"value":"hello"}""")), cancellationToken: TestContext.Current.CancellationToken);
        ...
        await Assert.ThrowsAsync<WorkflowExecutorInterruptedException>(
            async () => await runner.ExecuteAsync(
                ...
                new WorkflowRunRequest(
                    "recover",
                    "owner-a",
                    WorkflowTestData.Json("""{"value":"input"}""")), cancellationToken: TestContext.Current.CancellationToken));

        var runId = Assert.IsType<string>(
            adapter.LastInvocation?.WorkflowRunId);
        var started = await store.ReadAsync(runId, cancellationToken: TestContext.Current.CancellationToken);
        Assert.Equal(
            ...
            var execution = CreateRunner(
                ...
                .ExecuteAsync(
                    ...
                    new WorkflowRunRequest(
                        ...
                        WorkflowTestData.Json("""{"value":"input"}""")), cancellationToken: TestContext.Current.CancellationToken)
                .AsTask();
            await adapter.Entered.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);

            var failed = await execution.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
