...
public sealed class WorkflowRunnerTests
{
    ...
    public async Task IndependentRootsRunInParallelAndReduceInStableOrder()
    {
        ...
        var result = await runner.ExecuteAsync(
            ...
            new WorkflowRunRequest(
                "parallel-run",
                "owner-a",
                WorkflowTestData.Json("""{"seed":"x"}""")), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task StartedStepIsRecoveredWithANewGenerationAfterCrash()
    {
        ...
        var interrupted = await store.ReadAsync(runId, cancellationToken: TestContext.Current.CancellationToken);
        ...
        Assert.False(
            await executor.OriginalContext!.SaveCheckpointAsync(
                WorkflowTestData.Json("""{"late":true}"""), cancellationToken: TestContext.Current.CancellationToken));
    }
    ...
    public async Task WorkflowDeadlineFencesANonCooperativeExecutor()
    {
        ...
        var result = await execution.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        Assert.Equal(WorkflowRunStatus.Failed, result.Status);
        Assert.Equal(WorkflowReasonCodes.LimitExceeded, result.ReasonCode);
        ...
        await executor.Finished.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task CancellationOwnershipIsReservedBeforeExecutorDispatch()
    {
        ...
        try
        {
            await executor.AllEntered.Task.WaitAsync(TimeSpan.FromSeconds(5), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task CompletedInvocationCancelsOutstandingWorkflowTimers()
    {
        ...
        await executor.AllEntered.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        await delays.TwoCreated.Task.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        executor.ReleaseAll();
        var result = await execution.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        await WaitForConditionAsync(
            () => delays.Active == 0,
