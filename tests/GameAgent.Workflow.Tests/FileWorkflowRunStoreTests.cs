...
public sealed class FileWorkflowRunStoreTests
{
    ...
    public async Task CreateDuplicateAndReopenPreserveStableBytes()
    {
        ...
        var created = await store.CreateAsync(snapshot, cancellationToken: TestContext.Current.CancellationToken);
        var path = store.GetRunFilePath(snapshot.RunId);
        var firstBytes = File.ReadAllBytes(path);
        var duplicate = await store.CreateAsync(snapshot.Copy(), cancellationToken: TestContext.Current.CancellationToken);
        var secondBytes = File.ReadAllBytes(path);
        var reopened = await CreateStore(directory.Path)
            .ReadAsync(snapshot.RunId, cancellationToken: TestContext.Current.CancellationToken);
        var independentStore = CreateStore(independentDirectory.Path);
        Assert.Equal(snapshot.RunId, equivalentSnapshot.RunId);
        await independentStore.CreateAsync(equivalentSnapshot, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task StartedCheckpointRecoversAfterStoreReopen()
    {
        ...
        var persisted = await CreateStore(directory.Path).ReadAsync(runId, cancellationToken: TestContext.Current.CancellationToken);
        ...
        var recovered = await secondRunner.RecoverAsync(
            workflow,
            runId,
            "owner-b", cancellationToken: TestContext.Current.CancellationToken);
            ...
            async () => await runner.ExecuteAsync(
                ...
                new WorkflowRunRequest(
                    ...
                    WorkflowTestData.Json(
                        """{"value":0,"done":false}""")), cancellationToken: TestContext.Current.CancellationToken));

        var interrupted = await CreateStore(directory.Path)
            .ReadAsync(executor.RunId!, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task LeaseRenewTakeoverFenceAndCasRaceMatchMemorySemantics()
    {
        ...
        Assert.Equal(
            ...
            (await firstStore.CreateAsync(snapshot, cancellationToken: TestContext.Current.CancellationToken)).Status);
        ...
        Assert.Equal(
            ...
            (await secondStore.TryAcquireLeaseAsync(
                ...
                "owner-b",
                TimeSpan.FromSeconds(1),
                start.AddMilliseconds(1_200), cancellationToken: TestContext.Current.CancellationToken)).Status);
        ...
        var current = await secondStore.ReadAsync(snapshot.RunId, cancellationToken: TestContext.Current.CancellationToken);
        ...
        var stale = await firstStore.TryCommitAsync(
            ...
            first.Token,
            replacement,
            start.AddMilliseconds(1_700), cancellationToken: TestContext.Current.CancellationToken);
        ...
        var leftTask = firstStore.TryCommitAsync(
            ...
            takeover.Token,
            replacement,
            start.AddMilliseconds(1_700), cancellationToken: TestContext.Current.CancellationToken).AsTask();
    ...
    public async Task LostCommitAcknowledgementRetriesAsRevisionConflict()
    {
        ...
        await faultedStore.CreateAsync(snapshot, cancellationToken: TestContext.Current.CancellationToken);
        var start = DateTimeOffset.Parse("2026-01-01T00:00:00Z");
        var lease = await faultedStore.TryAcquireLeaseAsync(
            ...
            start, cancellationToken: TestContext.Current.CancellationToken);
        var current = await faultedStore.ReadAsync(snapshot.RunId, cancellationToken: TestContext.Current.CancellationToken);
        var replacement = SnapshotFactory.WithRevision(
            current!,
            current!.Revision + 1,
            WorkflowRunStatus.Running);
        ...
        await Assert.ThrowsAsync<InjectedStoreFaultException>(
            async () => await faultedStore.TryCommitAsync(
                ...
                lease.Token!,
                replacement,
                start.AddMilliseconds(1), cancellationToken: TestContext.Current.CancellationToken));
        ...
        var retry = await CreateStore(directory.Path).TryCommitAsync(
            ...
            lease.Token!,
            replacement,
            start.AddMilliseconds(2), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task TornTailIsIgnoredAndTruncatedByNextMutation()
    {
        ...
        var store = CreateStore(directory.Path);
        await store.CreateAsync(snapshot, cancellationToken: TestContext.Current.CancellationToken);
        var path = store.GetRunFilePath(snapshot.RunId);
        ...
        Assert.NotNull(await reopened.ReadAsync(snapshot.RunId, cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task ExclusiveWriterHonorsTimeoutAndCancellation()
    {
        ...
        await CreateStore(directory.Path).CreateAsync(snapshot, cancellationToken: TestContext.Current.CancellationToken);
        ...
        Assert.True(blocker.Entered.Wait(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken));
        ...
        await Assert.ThrowsAsync<WorkflowFileStoreLockTimeoutException>(
            async () => await shortStore.ReadAsync(snapshot.RunId, cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task CountOperationSnapshotAndFileLimitsLeaveNoPartialState()
    {
        ...
        Assert.Equal(
            WorkflowCreateStatus.Created,
            (await countStore.CreateAsync(first, cancellationToken: TestContext.Current.CancellationToken)).Status);
        Assert.Equal(
            WorkflowCreateStatus.CapacityExceeded,
            (await countStore.CreateAsync(second, cancellationToken: TestContext.Current.CancellationToken)).Status);
        Assert.Null(await countStore.ReadAsync(second.RunId, cancellationToken: TestContext.Current.CancellationToken));
        Assert.False(
        ...
        await operationStore.CreateAsync(operationSnapshot, cancellationToken: TestContext.Current.CancellationToken);
        await Assert.ThrowsAsync<WorkflowFileStoreCapacityException>(
            async () => await operationStore.TryAcquireLeaseAsync(
                ...
                DateTimeOffset.UtcNow, cancellationToken: TestContext.Current.CancellationToken));
        Assert.Null(
            (await operationStore.ReadAsync(operationSnapshot.RunId, cancellationToken: TestContext.Current.CancellationToken))!
                .Lease);
        ...
        var sizeStore = CreateStore(
            ...
            maxRootBytes: 1_184);
        await Assert.ThrowsAsync<WorkflowFileStoreCapacityException>(
            async () => await sizeStore.CreateAsync(oversized, cancellationToken: TestContext.Current.CancellationToken));
        Assert.Null(await sizeStore.ReadAsync(oversized.RunId, cancellationToken: TestContext.Current.CancellationToken));
        Assert.False(
        ...
        await fileStore.CreateAsync(fileSnapshot, cancellationToken: TestContext.Current.CancellationToken);
        var start = DateTimeOffset.Parse("2026-01-01T00:00:00Z");
            ...
            try
            {
                Assert.True(
                    await fileStore.RenewLeaseAsync(
                        ...
                        lease.Token!,
                        TimeSpan.FromSeconds(10),
                        start.AddMilliseconds(index), cancellationToken: TestContext.Current.CancellationToken));
            }
        ...
        Assert.InRange(
            ...
            1,
            maxFileBytes);
        Assert.NotNull(await fileStore.ReadAsync(fileSnapshot.RunId, cancellationToken: TestContext.Current.CancellationToken));
    }
        ...
        bool mayAlreadyBeCommitted)
    {
        ...
        await Assert.ThrowsAsync<InjectedStoreFaultException>(
            async () => await faulted.CreateAsync(snapshot, cancellationToken: TestContext.Current.CancellationToken));
        ...
        var retry = await CreateStore(directory.Path)
            .CreateAsync(snapshot, cancellationToken: TestContext.Current.CancellationToken);
        Assert.Contains(
            retry.Status,
            new[]
            {
    ...
    public async Task LostStartedAcknowledgementRecoversWithoutExecuteReplay()
    {
        ...
        var recovered = await new WorkflowRunner(
                CreateStore(directory.Path),
                new WorkflowStepExecutorRegistry(new[] { executor }))
            .RecoverAsync(workflow, runId, "owner-b", cancellationToken: TestContext.Current.CancellationToken);
