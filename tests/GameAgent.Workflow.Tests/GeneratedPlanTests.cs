...
public sealed class GeneratedPlanTests
{
    ...
    public async Task Generated_command_graph_runs_independent_steps_in_parallel_and_preserves_floats()
    {
        ...
        var result = await runner.ExecuteAsync(
            workflow,
            new WorkflowRunRequest("parallel-plan", "owner", WorkflowTestData.Json("{}")), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task Durable_receipt_prevents_side_effect_replay_after_interruption()
    {
        ...
        await Assert.ThrowsAsync<WorkflowExecutorInterruptedException>(
            async () => await runner.ExecuteAsync(
                workflow,
                new WorkflowRunRequest("receipt-run", "owner-a", WorkflowTestData.Json("{}")), cancellationToken: TestContext.Current.CancellationToken));
        var started = WorkflowIdentity.CreateRunId(
            ...
            "receipt-run");
        var recovered = await runner.RecoverAsync(workflow, started, "owner-b", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task External_attention_interrupts_workflow_until_durable_resolution()
    {
        ...
        await Assert.ThrowsAsync<WorkflowExecutorInterruptedException>(
            async () => await runner.ExecuteAsync(
                workflow,
                new WorkflowRunRequest("attention-run", "owner-a", WorkflowTestData.Json("{}")), cancellationToken: TestContext.Current.CancellationToken));
        var pending = Assert.Single(await attentionStore.ListPendingAsync(null, 10, TestContext.Current.CancellationToken));
        await attention.ResolveAsync(
            ...
            pending.Revision, cancellationToken: TestContext.Current.CancellationToken);
        var runId = WorkflowIdentity.CreateRunId(
            workflow.DefinitionDigest,
            WorkflowIdentity.ComputeJsonDigest(WorkflowTestData.Json("{}")),
            "attention-run");

        var recovered = await runner.RecoverAsync(workflow, runId, "owner-b", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task Generated_plan_can_fan_out_reduce_and_run_a_bounded_feedback_loop()
    {
        ...
        var result = await Runner(host).ExecuteAsync(
            ...
            new WorkflowRunRequest(
                "composite-run",
                "owner",
                WorkflowTestData.Json("{}")), cancellationToken: TestContext.Current.CancellationToken);
