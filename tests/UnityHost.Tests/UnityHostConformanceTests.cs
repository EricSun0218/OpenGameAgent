...
public sealed class UnityHostConformanceTests
{
    ...
    public async Task DispatcherWaitsForRunningActionAfterCancellation()
    {
        ...
        dispatcher.Pump(maxItems: 1, maxMilliseconds: 10);
        var actionToken = await started.Task.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
        cancellation.Cancel();
    ...
    public async Task DispatcherReportsThrowingCancellationCallbacksAndShutsDown()
    {
        ...
        await registered.Task.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
        dispatcher.Shutdown();

        await Assert.ThrowsAnyAsync<OperationCanceledException>(
            () => pending);
        var cancellationFailure = await observed.Task.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
    ...
    public async Task SynchronousBackendFailureCannotStrandShutdownSnapshot()
    {
        ...
        await backend.Started.Task.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);

        var shutdown = facade.ShutdownAsync(CancellationToken.None).AsTask();
        release.Set();
        await starting.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
        await shutdown.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
    ...
    public async Task FacadeCancelActiveRunsDoesNotRunCancellationCallbacksInline()
    {
        ...
        var caller = Task.Run(facade.CancelActiveRuns, TestContext.Current.CancellationToken);
        try
        {
            await backend.CancellationCallbackStarted.Task.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
            await caller.WaitAsync(TimeSpan.FromMilliseconds(250), TestContext.Current.CancellationToken);
        }
        finally
        {
            release.Set();
        ...
        await caller.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
        await Assert.ThrowsAnyAsync<OperationCanceledException>(() => run);
        await facade.ShutdownAsync(CancellationToken.None);
    }
    ...
    public async Task FacadeShutdownIsBoundedWhenCancellationCallbackBlocks()
    {
        ...
        try
        {
            ...
            var failure = await Assert.ThrowsAsync<AggregateException>(
                () => shutdown.WaitAsync(TimeSpan.FromSeconds(3), TestContext.Current.CancellationToken));
            Assert.Contains(
        ...
        await Assert.ThrowsAnyAsync<OperationCanceledException>(
            () => run.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken));
        Assert.True(
            SpinWait.SpinUntil(
        ...
        try
        {
            ...
            runs.Add(
                shutdownFacade.RunAsync(
                    new HeadlessRunRequest(),
                    CancellationToken.None));
            await shutdownBackend.Started.Task.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);

            shutdownFacade.CancelActiveRuns();
            Assert.Equal(1, UnityRunCancellationDispatcher.PendingCount);
            shutdownFacade.RequestShutdown();
            await shutdownBackend.CancellationCallbackStarted.Task.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
            Assert.Equal(
                UnityRunCancellationDispatcher.Capacity,
                UnityRunCancellationDispatcher.ActiveCount);
            Assert.Equal(
        ...
        FacadeLifecycleCancellationWaitsForCapacityWithoutNewWorker()
    {
        ...
        try
        {
            ...
            overflowRun = overflowFacade.RunAsync(
                new HeadlessRunRequest(),
                CancellationToken.None);
            await overflowBackend.Started.Task.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
            ...
            releases[0].Set();
            await overflowBackend.CancellationCallbackStarted.Task.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
            overflowRelease.Set();
        ...
        FacadeLifecycleLeasesRemainHeldUntilIgnoringRunsActuallyDrain()
    {
        ...
        try
        {
            ...
            try
            {
                await Task.WhenAll(shutdowns).WaitAsync(TimeSpan.FromSeconds(4), TestContext.Current.CancellationToken);
            }
            catch
            {
            }
        ...
        finally
        {
            ...
            try
            {
                await Task.WhenAll(runs).WaitAsync(TimeSpan.FromSeconds(3), TestContext.Current.CancellationToken);
            }
            catch
            {
            }
    ...
    public async Task FacadeExposesFailClosedGuardedResume()
    {
        ...
        var unsupported =
            await Assert.ThrowsAsync<DurableRunResumeGuardException>(
                () => unsupportedFacade.ResumeAsync("unsupported-run", guard, cancellationToken: TestContext.Current.CancellationToken));
        Assert.Equal(
    ...
    public async Task FacadeCreatesTrackedMultiActorCoordinatorWithLifecycle()
    {
        ...
        var outcome = await coordinator.RunAsync(new MultiActorDecisionBatch(
                "unity-batch",
    ...
    public async Task HostCoordinatorFactoryPreservesGuardedParticipantResume()
    {
        ...
        var batch = await coordinator.RunAsync(new MultiActorDecisionBatch(
                "unity-resume-batch",
                MultiActorCoordinate(),
                new[] { CreateMultiActorRequest(7) }), TestContext.Current.CancellationToken);
        ...
        var resumed = await coordinator.ResumeParticipantAsync(batch.BatchId, participant, semanticExpectation, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task HostCoordinatorFactoryPreservesGuardedAbandonment()
    {
        ...
        var batch = await coordinator.RunAsync(new MultiActorDecisionBatch(
                "unity-abandon-batch",
                MultiActorCoordinate(),
                new[] { CreateMultiActorRequest(8) }), TestContext.Current.CancellationToken);
        var participant = Assert.Single(batch.Manifest.Participants);

        var abandoned =
            await coordinator.ReconcileAbandonedParticipantAsync(batch.BatchId, participant, "actor_removed", cancellationToken: TestContext.Current.CancellationToken);
        ...
        TerminalQueueStopsNewReservationsButDrainsIssuedPublishers()
    {
        ...
        await publisherDrain.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
    ...
    public async Task HostApplicationQuitTimeoutSetsIncompleteState()
    {
        ...
        var running = StartBlockingDispatcherWork(
            ...
            workStarted,
            cancellationStarted);
        await workStarted.Task.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
        ...
        await Assert.ThrowsAnyAsync<OperationCanceledException>(
            () => running.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken));
        await host.ShutdownAsync(CancellationToken.None)
            .WaitAsync(TimeSpan.FromSeconds(3), TestContext.Current.CancellationToken);
    ...
    public async Task HostShutdownWaitsForStartedDispatcherWorkBeforeDisposal()
    {
        ...
        var handlerToken = await started.Task.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
        ...
        HostDefersOwnedResourceCleanupAfterDispatcherDrainTimeout()
    {
        ...
        release.TrySetResult(9);
        Assert.Equal(9, await running.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken));
        await host
            .ShutdownAsync(CancellationToken.None)
            .WaitAsync(TimeSpan.FromSeconds(3), TestContext.Current.CancellationToken);
        ...
        HostShutdownIsBoundedAndRetriableWhenDispatcherCancellationBlocks()
    {
        ...
        try
        {
            await cancellationStarted.Task.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
            var failure = await Assert.ThrowsAsync<AggregateException>(
                () => shutdown.WaitAsync(TimeSpan.FromSeconds(4), TestContext.Current.CancellationToken));
            Assert.Contains(
                failure.Flatten().InnerExceptions,
                exception => exception is TimeoutException);
        }
        ...
        HostOnDestroyQueuesLifecycleCancellationAcrossProcessSaturation()
    {
        ...
        try
        {
            ...
            await overflowWorkStarted.Task.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
            ...
            releases[0].Set();
            await overflowCancellationStarted.Task.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
            overflowRelease.Set();
            await Assert.ThrowsAnyAsync<OperationCanceledException>(
                () => overflowRunning.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken));
            await overflowHost
                .ShutdownAsync(CancellationToken.None)
                .WaitAsync(TimeSpan.FromSeconds(3), TestContext.Current.CancellationToken);
    ...
    public async Task HostCallerCancellationDoesNotPoisonSharedShutdown()
    {
        ...
        await store.FlushStarted.Task.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
