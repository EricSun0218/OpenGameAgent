...
public sealed class MixedAtomicBatchTests
{
    ...
    public async Task AllFreshAndAllDuplicateActionBatchesRemainSupported()
    {
        ...
        try
        {
            ...
            var after = await store.GetRunCursorAsync(run.RunId, TestContext.Current.CancellationToken);
            ...
            Assert.NotNull(
                await store.GetOperationAsync(first.OperationId, TestContext.Current.CancellationToken));
            Assert.NotNull(
                await store.GetOperationAsync(second.OperationId, TestContext.Current.CancellationToken));
        }
        finally
        {
            DeleteJournal(path);
