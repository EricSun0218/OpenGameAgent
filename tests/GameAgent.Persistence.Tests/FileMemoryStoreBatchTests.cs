...
public sealed class FileMemoryStoreBatchTests
{
                             ...
                             }))
            {
                ...
                var result =
                    await store.ApplyAtomicBatchWithRevisionAsync(
                        ...
                        expectedRevision: 1, cancellationToken: TestContext.Current.CancellationToken);
        ...
        try
        {
            ...
            await using (var store = new FileMemoryStore(path))
            {
                var first = await store.ApplyIdempotentAtomicBatchAsync(
                    commitId,
                    firstBatch, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task RecoveryRejectsIdempotentFrameWithForgedPayloadDigest()
    {
        ...
        try
        {
            ...
            var frame = await File.ReadAllBytesAsync(path, cancellationToken: TestContext.Current.CancellationToken);
            ...
            await File.WriteAllBytesAsync(path, frame, cancellationToken: TestContext.Current.CancellationToken);
            ...
            var error =
                await Assert.ThrowsAsync<
                    ...
                    () => store.ApplyAtomicBatchWithRevisionAsync(
                            ...
                            expectedRevision: 0, cancellationToken: TestContext.Current.CancellationToken)
                        .AsTask());
        ...
        try
        {
                             ...
                             }))
            {
                ...
                await Assert.ThrowsAsync<MemoryStoreFaultedException>(
                    () => faulted.GetRevisionAsync(cancellationToken: TestContext.Current.CancellationToken).AsTask());
            }
    ...
    public async Task VersionOneBareReplacementReplaysBeforeVersionTwoWrites()
    {
        ...
        try
        {
            ...
            Buffer.BlockCopy(
                ...
                create.Length,
                replace.Length);
            await File.WriteAllBytesAsync(path, log, cancellationToken: TestContext.Current.CancellationToken);
        ...
        try
        {
            await File.WriteAllBytesAsync(
                ...
                LegacyFrame(
                    ...
                    contentVersion: 1,
                    formatVersion: 2,
                    mutationContractVersion: 0), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task FileStoreConditionalReplacementSurvivesRecovery()
    {
        ...
        try
        {
            ...
            var stale = await Assert.ThrowsAsync<
                ...
                () => recovered.ApplyIdempotentAtomicBatchAsync(
                        "stale-replacement",
                        new[] { MemoryMutation.Upsert(original, original) }, cancellationToken: TestContext.Current.CancellationToken)
                    .AsTask());
            Assert.Equal(
                MemoryBatchReasonCodes.PreconditionFailed,
                stale.ReasonCode);
            ...
            foreach (var candidate in foreign)
            {
                ...
                var delete = await Assert.ThrowsAsync<
                    ...
                    () => recovered.ApplyAtomicBatchAsync(
                            new[] { MemoryMutation.Delete(candidate) }, cancellationToken: TestContext.Current.CancellationToken)
                        .AsTask());
                Assert.Equal(
                    MemoryBatchReasonCodes.PreconditionFailed,
                    delete.ReasonCode);
