...
public sealed class SkillAdmissionTests
{
    ...
    public async Task UntrustedActiveSkillFailsBeforeSystemDisclosureOrProviderDispatch()
    {
        ...
        var outcome = await rig.Runtime.RunAsync(
            Request(run, "unsafe-skill"), cancellationToken: TestContext.Current.CancellationToken);
        ...
        string reasonCode)
    {
        ...
        var outcome = await rig.Runtime.RunAsync(
            Request(Run(), "declared-skill"), cancellationToken: TestContext.Current.CancellationToken);
        ...
        string reasonCode)
    {
        ...
        var outcome = await rig.Runtime.RunAsync(
            Request(Run(), "lookup-skill"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task CustomPolicyCanExplicitlyAdmitDeclarationsTheDefaultDoesNotInterpret()
    {
        ...
        var outcome = await rig.Runtime.RunAsync(
            Request(run, "game-approved-skill"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task CustomPolicyCannotBypassRequiredToolInvariant()
    {
        ...
        var outcome = await rig.Runtime.RunAsync(
            Request(Run(), "missing-tool-skill"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ThrowingCustomPolicyFailsClosedBeforeProviderDispatch()
    {
        ...
        var outcome = await rig.Runtime.RunAsync(
            Request(Run(), "policy-error-skill"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task InactiveCatalogHotUpdateWaitsForNextLoopInvocation()
    {
        ...
        var nextOutcome = await rig.Runtime.RunAsync(
            new DurableRunRequest { Run = nextRun }, cancellationToken: TestContext.Current.CancellationToken);
        ...
        var snapshots = (await rig.Store.ReadRunAsync(run.RunId, TestContext.Current.CancellationToken))
    ...
    [Fact]
    public async Task StableSkillPrefixIsFirstAndSeparateFromDynamicPromptDigest()
    {
        await using var rig = new RuntimeRig(
            new[] { Tool("advance_turn", "1.0.0") },
            new[]
        ...
        var outcome = await rig.Runtime.RunAsync(
            Request(run, "stable-skill"), cancellationToken: TestContext.Current.CancellationToken);

        Assert.Equal(RunStates.Completed, outcome.Run.State);
        Assert.Equal(2, rig.Provider.Requests.Count);
        Assert.Equal(
            NormalizedRoles.System,
            rig.Provider.Requests[0].Messages[0].Role);
        Assert.Equal(
            NormalizedRoles.System,
            rig.Provider.Requests[1].Messages[0].Role);
        ...
        Assert.Equal(2, snapshots.Length);
        Assert.Equal(
            snapshots[0].GetProperty("stablePrefixHash").GetString(),
            snapshots[1].GetProperty("stablePrefixHash").GetString());
        Assert.NotEqual(
            snapshots[0].GetProperty("extensions")
                .GetProperty("promptDigest")
                .GetString(),
            snapshots[1].GetProperty("extensions")
                .GetProperty("promptDigest")
                .GetString());
        Assert.All(
            snapshots,
            snapshot => Assert.Equal(
                1,
                snapshot.GetProperty("extensions")
                    .GetProperty("stablePrefixMessageCount")
                    .GetInt32()));
    }
    ...
    public async Task ResumeWithoutActiveSkillDoesNotReplayPriorSkillInstructions()
    {
        ...
        var initial = await rig.Runtime.RunAsync(
            Request(run, "resume-skill"), cancellationToken: TestContext.Current.CancellationToken);
        ...
        var resumed = await rig.Runtime.ResumeAsync(
            ...
            new SucceedingReconciler(), cancellationToken: TestContext.Current.CancellationToken);
