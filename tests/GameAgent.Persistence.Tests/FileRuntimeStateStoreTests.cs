...
public sealed class FileRuntimeStateStoreTests
{
    ...
    public async Task All_runtime_state_stores_recover_committed_state()
    {
        ...
        try
        {
            ...
            {
                await store.PutAsync(Baseline("actor:one", 3.5), null, TestContext.Current.CancellationToken);
            }

            await using (var recovered = new FileModelContextSectionBaselineStore(baselinePath))
            {
                var baseline = await recovered.TryGetAsync("actor:one", TestContext.Current.CancellationToken);
            ...
            await using (var store = new FileExternalAttentionStore(attentionPath))
            {
                ...
                await coordinator.RequestAsync(Attention("ask-1"), cancellationToken: TestContext.Current.CancellationToken);
            }

            await using (var recovered = new FileExternalAttentionStore(attentionPath))
            {
                var pending = await recovered.ListPendingAsync("world", 10, TestContext.Current.CancellationToken);
            ...
            {
                Assert.Equal(12, (await recovered.TryGetAsync("month:world", TestContext.Current.CancellationToken))!
            ...
            {
                var state = await recovered.ReadAsync(TestContext.Current.CancellationToken);
            ...
            await using (var store = new FilePersistentAgentGraphStore(graphPath))
            {
                ...
                await graph.EnqueueAsync("npc", Mail("message", 2.75), cancellationToken: TestContext.Current.CancellationToken);
            }

            await using (var recovered = new FilePersistentAgentGraphStore(graphPath))
            {
                var node = await new PersistentAgentGraph(recovered).TryGetAsync("npc", cancellationToken: TestContext.Current.CancellationToken);
            ...
            {
                await store.PutAsync(Distilled("distill-1"), null, TestContext.Current.CancellationToken);
            }

            await using (var recovered = new FileMemoryDistillationStore(distillationPath))
            {
                var record = await recovered.TryGetAsync("distill-1", TestContext.Current.CancellationToken);
    ...
    public async Task Torn_tail_is_truncated_to_last_fully_flushed_frame()
    {
        ...
        try
        {
            ...
            {
                await new PersistentAgentGraph(store).RegisterAsync(Node("npc"), cancellationToken: TestContext.Current.CancellationToken);
            }
            ...
            await using (var tail = new FileStream(
                             ...
                             FileShare.None))
            {
                await tail.WriteAsync(new byte[] { 0x47, 0x41, 0x53 }, cancellationToken: TestContext.Current.CancellationToken);
                await tail.FlushAsync(cancellationToken: TestContext.Current.CancellationToken);
            }
            Assert.True(new FileInfo(path).Length > committedLength);

            await using var recovered = new FilePersistentAgentGraphStore(path);
            Assert.NotNull(await new PersistentAgentGraph(recovered).TryGetAsync("npc", cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task Interior_corruption_is_rejected_instead_of_silently_rolled_back()
    {
        ...
        try
        {
            ...
            {
                var graph = new PersistentAgentGraph(store);
                await graph.RegisterAsync(Node("a"), cancellationToken: TestContext.Current.CancellationToken);
                await graph.RegisterAsync(Node("b"), cancellationToken: TestContext.Current.CancellationToken);
            }
