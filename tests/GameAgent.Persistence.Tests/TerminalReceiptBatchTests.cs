...
public sealed class TerminalReceiptBatchTests
{
        ...
        try
        {
            await using (var store = new FileSessionStore(path))
            {
                ...
                var append = await store.AppendAtomicBatchAsync(
                    ...
                    expectedRunRevision: run.Revision, cancellationToken: TestContext.Current.CancellationToken);
            ...
            var recoveredOperation = await recovered.GetOperationAsync(
                "operation-1", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task FreshTerminalBatchRejectsAuthoritativeMismatchAtomically()
    {
        ...
        try
        {
            ...
            Assert.Equal(
                run.Revision,
                (await store.GetRunCursorAsync(run.RunId, cancellationToken: TestContext.Current.CancellationToken)).Revision);
            var operation = await store.GetOperationAsync(request.OperationId, cancellationToken: TestContext.Current.CancellationToken);
            Assert.NotNull(operation);
