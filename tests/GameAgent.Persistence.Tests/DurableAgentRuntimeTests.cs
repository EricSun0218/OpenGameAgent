...
public sealed class DurableAgentRuntimeTests
{
    ...
    [Fact]
    public async Task OrdinaryStreamingEventsDoNotClaimProvisionalAdmissionState()
    {
        ...
        try
        {
            ...
            var provider = new QueueStreamingProvider(
                ToolResponse(
                    ...
                    """{"entityId":"npc-7"}"""),
                FinalResponse("done"));
            await using var runtime = CreateRuntimeWithPublisher(
                store,
                provider,
                new Host(
            ...
            Assert.Contains(
                deltas,
                item => item.Payload.GetProperty("kind").GetString()
    ...
    public async Task OversizedRunCollectionIsRejectedBeforeValidation()
    {
        ...
        try
        {
            ...
            var error = await Assert.ThrowsAsync<RuntimeContentLimitException>(
                () => runtime.RunAsync(
                        new DurableRunRequest { Run = rejectedRun }, cancellationToken: TestContext.Current.CancellationToken)
                    .AsTask());

            Assert.Equal("agent_run_items_exceeded", error.LimitCode);
            Assert.Empty(
                await store.ReadRunAsync(rejectedRun.RunId, TestContext.Current.CancellationToken));
    ...
    public async Task NewRunRejectsNonFreshSnapshotsBeforeAnySideEffect()
    {
        ...
        try
        {
            ...
            foreach (var mutate in mutations)
            {
                ...
                Assert.Empty(
                    await store.ReadRunAsync(run.RunId, TestContext.Current.CancellationToken));
            }
        ...
        bool ordinaryRun)
    {
        ...
        try
        {
            ...
            var eventCount = (await store.ReadRunAsync(run.RunId, TestContext.Current.CancellationToken))
                .Count;
            var provider = new QueueStreamingProvider(
            ...
            Assert.Equal(
                eventCount,
                (await store.ReadRunAsync(run.RunId, TestContext.Current.CancellationToken)).Count);
        }
        finally
        {
            Directory.Delete(directory, recursive: true);
        ...
        try
        {
            ...
            var error =
                await Assert.ThrowsAsync<DurableRunResumeGuardException>(
                    () => runtime.ResumeAsync(
                            run.RunId,
                            guard: staleExpectation, cancellationToken: TestContext.Current.CancellationToken)
                        .AsTask());
    ...
    public async Task RequiredSemanticGuardRejectsUnguardedNonterminalResume()
    {
        ...
        try
        {
            ...
            var error =
                await Assert.ThrowsAsync<DurableRunResumeGuardException>(
                    () => runtimeView.ResumeAsync(run.RunId, cancellationToken: TestContext.Current.CancellationToken).AsTask());
    ...
    public async Task RequiredSemanticGuardAllowsUnguardedTerminalReplay()
    {
        ...
        try
        {
            ...
            var replayed = await runtimeView.ResumeAsync(run.RunId, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ResumeCancellationIsPersistedBeforeTheProviderLoop()
    {
        ...
        try
        {
            ...
            var recovered = await legacyView.ResumeAsync(run.RunId, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ResumeCancellationWithPendingOperationOnlyReconciles()
    {
        ...
        try
        {
            ...
            var cancelled = await recoveredRuntime.ResumeAsync(
                first.Run.RunId,
                reconciler: new Reconciler(clock.UtcNow), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ConcurrentResumeAndResumeCancellationShareOwnership()
    {
        ...
        try
        {
            ...
            runtime = CreateRuntime(
                ...
                clock,
                ids);
            var activeResume = runtime.ResumeAsync(run.RunId, cancellationToken: TestContext.Current.CancellationToken).AsTask();
            await provider.Started.Task.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
            ...
            _ = await activeResume.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
        }
        finally
        {
            provider.Release.TrySetResult();
    ...
    public async Task NewRunInitializationIsCommittedAsOneOrderedBatch()
    {
        ...
        try
        {
            ...
            var events = await inner.ReadRunAsync(run.RunId, TestContext.Current.CancellationToken);
            Assert.Equal(
                RunStates.Preparing,
    ...
    public async Task RunStartCrashRetainsInitialContextAndActiveSkill()
    {
        ...
        try
        {
            ...
            var afterCrash = await inner.ReadRunAsync(run.RunId, TestContext.Current.CancellationToken);
            Assert.Equal(
                new[]
                {
                    RuntimeEventKinds.RunStarted,
            ...
            var resumed = await resumedRuntime.ResumeAsync(run.RunId, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task PartialLegacyInitializationFailsClosedWithoutProviderCall()
    {
        ...
        try
        {
            ...
            await journal.CommitTransitionAsync(
                run,
                RunStates.Preparing,
                RuntimeEventKinds.RunStarted, cancellationToken: TestContext.Current.CancellationToken);
            ...
            var outcome = await runtime.ResumeAsync(run.RunId, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task PreProviderTurnIsAbandonedAndRefundedBeforeResume()
    {
        ...
        try
        {
            ...
            await journal.CommitRunMutationAsync(
                ...
                abandonedTurnId,
                attemptId: "abandoned-attempt", cancellationToken: TestContext.Current.CancellationToken);
            await journal.AppendTranscriptAsync(
                run,
                InitialMessage(
                    "durable-turn-input",
            ...
            var events = await store.ReadRunAsync(run.RunId, TestContext.Current.CancellationToken);
    ...
    public async Task SnapshotCrashRetainsRequiredContextAndActiveSkillOnResume()
    {
        ...
        try
        {
            ...
            var preparedEvents = await inner.ReadRunAsync(run.RunId, TestContext.Current.CancellationToken);
    ...
    public async Task PreProviderTurnRefundSurvivesAnotherRecovery()
    {
        ...
        try
        {
                         ...
                         new ThrowAfterPreProviderAbandonStore(inner))
            {
                ...
                var interrupted = await firstRuntime.ResumeAsync(run.RunId, cancellationToken: TestContext.Current.CancellationToken);
            ...
            var recovered = await recoveredRuntime.ResumeAsync(run.RunId, cancellationToken: TestContext.Current.CancellationToken);
            ...
            Assert.Contains(
                ...
                part => part.Text?.Contains(
                    ...
                    StringComparison.Ordinal) == true);
            Assert.Single(
                await inner.ReadRunAsync(run.RunId, TestContext.Current.CancellationToken),
                item => item.Kind == RuntimeEventKinds.TurnCompleted
                        && item.ReasonCode
                        == RunRecovery.ReplaySafeTurnAbandonedReason);
        }
    ...
    public async Task KnownZeroDispatchTurnIsSafelyRefundedBeforeResume()
    {
        ...
        try
        {
            ...
            await journal.CommitRunMutationAsync(
                ...
                turnId,
                attemptId: "turn-attempt", cancellationToken: TestContext.Current.CancellationToken);
            await journal.CommitRunMutationAsync(
                run,
    ...
    public async Task RunsTypedContextThroughJournaledToolLoopToFinal()
    {
        ...
        try
        {
            ...
            var events = await store.ReadRunAsync(outcome.Run.RunId, TestContext.Current.CancellationToken);
            Assert.True(
                Array.FindIndex(
                    events.ToArray(),
                    item => item.Kind == RuntimeEventKinds.ActionRequested)
    ...
    public async Task ExplicitNonSecretContinuationRestoresAcrossRestart()
    {
        ...
        try
        {
            ...
            var finalEvents = await inner.ReadRunAsync(run.RunId, TestContext.Current.CancellationToken);
            Assert.Single(
                finalEvents,
                item => item.Extensions.ContainsKey(
                    ProviderOpaqueContinuationState.JournalExtensionName));
    ...
    public async Task ResumeSelectsDeferredContextWhenContinuationResuppliesIt()
    {
        ...
        try
        {
            ...
            var recovered = await recoveredRuntime.ResumeAsync(
                ...
                new Reconciler(clock.UtcNow), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task UnknownActionRequiresReconciliationAndNeverRedispatches()
    {
        ...
        try
        {
            ...
            Assert.Contains(
                ...
                message => message.Role == NormalizedRoles.Tool);
            Assert.Contains(
                await store.ReadRunAsync(first.Run.RunId, TestContext.Current.CancellationToken),
                item => item.Kind == RuntimeEventKinds.ToolCompleted);
        }
        finally
        {
    ...
    public async Task SteerAfterAccountedUsageCancelsStaleStreamAndStartsNewTurn()
    {
        ...
        try
        {
            ...
            var running = runtime.RunAsync(
                new DurableRunRequest { Run = run }, cancellationToken: TestContext.Current.CancellationToken).AsTask();
            ...
            var outcome = await running.WaitAsync(TimeSpan.FromSeconds(5), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task StrictObservationIncarnationRejectsControlBeforeInterruptOrJournal()
    {
        ...
        try
        {
            ...
            Assert.Single(provider.Requests);
            cancellation.Cancel();
            _ = await running.WaitAsync(TimeSpan.FromSeconds(5), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ProviderCompletionRacingSteerClosesTheDiscardedDispatch()
    {
        ...
        try
        {
            ...
            var loaded = await new RunRecovery(
                    ...
                    store,
                    recoveryJournal)
                .LoadAsync(run.RunId, TestContext.Current.CancellationToken);
            Assert.NotNull(loaded);
            Assert.Empty(loaded!.UnsettledProviderDispatches);

            var resumed = await runtime.ResumeAsync(run.RunId, cancellationToken: TestContext.Current.CancellationToken);
            Assert.Equal(RunStates.Completed, resumed.Run.State);
    ...
    public async Task CancellationCannotEraseObservedProviderUsage()
    {
        ...
        try
        {
            ...
            await intercept.ProviderUsageAppendStarted.Task.WaitAsync(
                TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
            cancellation.Cancel();
            intercept.ReleaseProviderUsageAppend.TrySetResult();

            var outcome = await pending.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task CancellationBeforeUsagePersistsUnaccountedUsageAcrossRecovery()
    {
        ...
        try
        {
            ...
            var pending = runtime.RunAsync(
                    ...
                    cancellation.Token)
                .AsTask();
            await provider.Started.Task.WaitAsync(TimeSpan.FromSeconds(10), cancellationToken: TestContext.Current.CancellationToken);
            cancellation.Cancel();
            var outcome = await pending.WaitAsync(TimeSpan.FromSeconds(10), cancellationToken: TestContext.Current.CancellationToken);
            ...
            var recovered = await runtime.ResumeAsync(run.RunId, cancellationToken: TestContext.Current.CancellationToken);
        ...
        try
        {
                       ...
                       ids))
            {
                ...
                await journal.CommitRunMutationAsync(
                    ...
                    providerId: "cancel-provider-uncertain", cancellationToken: TestContext.Current.CancellationToken);
                await journal.CommitRunMutationAsync(
                    run,
                    RuntimeEventKinds.ProviderDispatchStarted,
                    _ => { },
                    ...
                    providerId: "cancel-provider-billed", cancellationToken: TestContext.Current.CancellationToken);
                await journal.CommitRunMutationAsync(
                    run,
                       ...
                       ids))
            {
                ...
                if (!requestCancellation)
                {
                    await journal.CommitTransitionAsync(
                        ...
                        completionIntent: CompletionIntents.Failed, cancellationToken: TestContext.Current.CancellationToken);
                }
            }
        ...
        try
        {
                       ...
                       ids))
            {
                ...
                await journal.CommitRunMutationAsync(
                    ...
                    providerId: "terminal-provider-uncertain", cancellationToken: TestContext.Current.CancellationToken);
                await journal.CommitRunMutationAsync(
                    run,
                    RuntimeEventKinds.ProviderDispatchStarted,
                    _ => { },
                    ...
                    providerId: "terminal-provider-billed", cancellationToken: TestContext.Current.CancellationToken);
                await journal.CommitRunMutationAsync(
                    run,
                    RuntimeEventKinds.BudgetUpdated,
                    next =>
                ...
                await journal.CommitTransitionAsync(
                    ...
                    turnId: turnId, cancellationToken: TestContext.Current.CancellationToken);
            }
            ...
            var recovered = await runtime.ResumeAsync(
                run.RunId,
                continuation, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task InitialTranscriptIsRejectedBeforeJournalWhenPromptIsTooLarge()
    {
        ...
        try
        {
            ...
            Assert.Empty(await store.ReadRunAsync(run.RunId, TestContext.Current.CancellationToken));
        }
        finally
        {
            Directory.Delete(directory, recursive: true);
    ...
    public async Task BackgroundRunsCannotConsumeTheInteractiveProviderSlot()
    {
        ...
        try
        {
            ...
            var background = Enumerable.Range(0, 3)
                ...
                .ToArray();
            await provider.TwoEntered.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
            ...
            var interactive = runtime.RunAsync(
                    new DurableRunRequest { Run = interactiveRun }, cancellationToken: TestContext.Current.CancellationToken)
                .AsTask();
            await provider.ThreeEntered.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task RunEntryDeepSnapshotsMutableRequestGraphBeforeWaiting()
    {
        ...
        try
        {
            ...
            var pending = runtime.RunAsync(request, cancellationToken: TestContext.Current.CancellationToken).AsTask();
            ...
            await holding.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
            var outcome = await pending.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ResumeEntryDeepSnapshotsMutableContinuationBeforeWaiting()
    {
        ...
        try
        {
            ...
            var initial = await runtime.RunAsync(
                new DurableRunRequest { Run = targetRun }, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task DurationDeadlineLeavesDispatchedSlowToolForReconciliation()
    {
        ...
        try
        {
            ...
            var execution = runtime.RunAsync(
                    new DurableRunRequest { Run = run }, cancellationToken: TestContext.Current.CancellationToken)
                .AsTask();
            await host.Started.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
            var outcome = await execution.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
    ...
    {
        var directory = TempDirectory();
        ControllableSlowHost? host = null;
        try
        {
            await using var store = new FileSessionStore(
                Path.Combine(directory, "runtime.journal"));
            ...
            host = new ControllableSlowHost();
            var toolTimeout = new TriggeredRuntimeDelay();
            await using var runtime = CreateRuntimeCoreWithSkills(
                store,
                store,
                provider,
            ...
            var execution = runtime.RunAsync(
                    ...
                    cancellationToken: TestContext.Current.CancellationToken)
                .AsTask();
            await host.Started.WaitAsync(
                TestWaitTimeout,
                TestContext.Current.CancellationToken);
            toolTimeout.Trigger();
            var initial = await execution.WaitAsync(
                TestWaitTimeout,
                TestContext.Current.CancellationToken);
            host.Release();
            await host.Settled.WaitAsync(
                TestWaitTimeout,
                TestContext.Current.CancellationToken);

            Assert.Equal(RunStates.Reconciling, initial.Run.State);
            Assert.Single(initial.Run.PendingOperationIds);
            var before = await store.ReadRunAsync(run.RunId, TestContext.Current.CancellationToken);
            Assert.Contains(
                before,
                item => item.Kind
                        == RuntimeEventKinds.ActionOutcomeUncertain);
            ...
            var resumed = await runtime.ResumeAsync(
                run.RunId,
                reconciler: new RevisionZeroReconciler(clock.UtcNow), cancellationToken: TestContext.Current.CancellationToken);

            Assert.Equal(RunStates.Completed, resumed.Run.State);
            Assert.Empty(resumed.Run.PendingOperationIds);
            Assert.Equal(2, provider.Requests.Count);
            var after = await store.ReadRunAsync(run.RunId, TestContext.Current.CancellationToken);
    ...
    public async Task ExternalCancellationDuringReconciliationPersistsIntent()
    {
        ...
        try
        {
            ...
            var cancelling = runtime.ResumeAsync(
                ...
                .AsTask();
            await reconciler.Started.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
            cancellation.Cancel();

            var fenced = await cancelling.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
            ...
            reconciler.Release();
            await reconciler.Completed.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
            var cancelled = await runtime.ResumeAsync(
                initial.Run.RunId,
                reconciler: new Reconciler(clock.UtcNow), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task DurationDeadlineReconciliationSurvivesRepeatedReopenAndResume()
    {
        ...
        try
        {
            ...
            for (var reopen = 0; reopen < 2; reopen++)
            {
                ...
                var outcome = await runtime.ResumeAsync(runId, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task DurationDeadlineAndStopIgnoreBlockingCancellationCallbacks()
    {
        ...
        try
        {
            ...
            var running = runtime.RunAsync(
                    new DurableRunRequest { Run = run }, cancellationToken: TestContext.Current.CancellationToken)
                .AsTask();
            await delay.Started.Task.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
            var outcome = await running
                .WaitAsync(TimeSpan.FromSeconds(10), cancellationToken: TestContext.Current.CancellationToken);
            ...
            await runtime.StopAsync().AsTask().WaitAsync(
                TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
            var actualDrain = runtime.WaitForShutdownDrainAsync(cancellationToken: TestContext.Current.CancellationToken).AsTask();
            Assert.False(actualDrain.IsCompleted);
            delay.Release.TrySetResult();
            await actualDrain.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
        }
        finally
        {
            delay.Release.TrySetResult();
    ...
    public async Task DeadlineWithBlockingProviderCancellationStillStops()
    {
        ...
        try
        {
            ...
            var running = runtime.RunAsync(
                ...
                .AsTask();
            await provider.Started.Task.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
            var outcome = await running.WaitAsync(TimeSpan.FromSeconds(3), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task StopDoesNotWaitForBlockingProviderCancellationCallback()
    {
        ...
        try
        {
            ...
            await runtime.StopAsync().AsTask().WaitAsync(
                TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
            var outcome = await running.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);

            Assert.Equal(RunStates.Cancelled, outcome.Run.State);
            await provider.CallbackInvoked.Task.WaitAsync(
                TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
            var actualDrain = runtime.WaitForShutdownDrainAsync(cancellationToken: TestContext.Current.CancellationToken).AsTask();
            await Task.Delay(TimeSpan.FromMilliseconds(50), cancellationToken: TestContext.Current.CancellationToken);
            Assert.False(actualDrain.IsCompleted);
    ...
    public async Task DisposeWaitsForActiveLeaseBeyondBoundedStop()
    {
        ...
        try
        {
            ...
            var running = runtime.RunAsync(
                    new DurableRunRequest { Run = Run(clock.UtcNow) }, cancellationToken: TestContext.Current.CancellationToken)
                .AsTask();
            await policy.SelectionEntered.Task.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);

            await runtime.StopAsync().AsTask().WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
            ...
            _ = await running.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
            await dispose.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
            Assert.True(runtime.ShutdownResourceCleanupCompleted);
        }
        finally
        {
            ...
            await memory.WaitForShutdownDrainAsync(cancellationToken: TestContext.Current.CancellationToken);
            Directory.Delete(directory, recursive: true);
        }
    }
    ...
    public async Task BlockingShutdownCallbacksConsumeOnlyBoundedCapacity()
    {
        ...
        try
        {
            ...
            await first.StopAsync().AsTask().WaitAsync(
                TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
            await callbackInvoked.Task.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
            var firstActualDrain =
                first.WaitForShutdownDrainAsync(cancellationToken: TestContext.Current.CancellationToken).AsTask();
            await Task.Delay(TimeSpan.FromMilliseconds(50), cancellationToken: TestContext.Current.CancellationToken);
            ...
            await firstActualDrain.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task DetachedPreparationDoesNotStarveGlobalProviderAdmission()
    {
        ...
        try
        {
            ...
            var failed = runtime.RunAsync(
                    new DurableRunRequest { Run = Run(clock.UtcNow) }, cancellationToken: TestContext.Current.CancellationToken)
                .AsTask();
            await first.Entered.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
            await second.Entered.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
            var failedOutcome = await failed.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
            ...
            var next = runtime.RunAsync(
                    new DurableRunRequest { Run = Run(clock.UtcNow) }, cancellationToken: TestContext.Current.CancellationToken)
                .AsTask();
            var completed = await next.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
            Assert.True(
                string.Equals(
                    RunStates.Completed,
                    completed.Run.State,
            ...
            first.Release();
            await first.Settled.WaitAsync(TestWaitTimeout, cancellationToken: TestContext.Current.CancellationToken);
        }
        finally
        {
            first.Release();
        ...
        try
        {
            ...
            await new JournalCoordinator(
                ...
                .AppendBuiltInDurableAsync(
                    ...
                    attemptId: "deadline-test", cancellationToken: TestContext.Current.CancellationToken);

            var resumed = await runtime.ResumeAsync(
                    run.RunId,
                    reconciler: new SlowReconciler(), cancellationToken: TestContext.Current.CancellationToken)
                .AsTask()
                .WaitAsync(TimeSpan.FromSeconds(3), cancellationToken: TestContext.Current.CancellationToken);
    ...
    private static DurableAgentRuntime CreateRuntimeWithPublisher(
        FileSessionStore store,
        IStreamingModelProvider provider,
        IGameHost host,
        IRuntimeClock clock,
    ...
    private static DurableAgentRuntime CreateRuntimeCoreWithSkills(
        ...
        INonBlockingRuntimeEventPublisher? eventPublisher = null)
    {
...
#pragma warning restore CS0162
    ...
    private sealed class TriggeredRuntimeDelay : IRuntimeDelay
    {
        private readonly TaskCompletionSource _triggered =
            new(TaskCreationOptions.RunContinuationsAsynchronously);
    ...
    private sealed class ControllableSlowHost : IGameHost
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                await _release.Task.ConfigureAwait(false);
                throw new InvalidOperationException(
                    "The controlled host was released after detachment.");
            }
            finally
            {
                _settled.TrySetResult();
