...
public sealed class FileSessionStoreTests
{
    ...
    public async Task SharedReaderCanReadWhileJournalWriterIsActive()
    {
        ...
        try
        {
            ...
            await store.FlushAsync(cancellationToken: TestContext.Current.CancellationToken);
            ...
            await reader.CopyToAsync(copy, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task AppendAssignsPerRunSequenceAndRevisionAndRecoversThem()
    {
        ...
        try
        {
            ...
            {
                var cursor = await recovered.GetRunCursorAsync("run-a", cancellationToken: TestContext.Current.CancellationToken);
                ...
                var third = await recovered.AppendAtomicAsync(
                    CreateEvent("event-a3", "run-a"),
                    expectedRunRevision: 2, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ConcurrentAppendsAreSerializedWithoutSequenceGaps()
    {
        ...
        try
        {
            ...
            await using (var recovered = new FileSessionStore(path))
            {
                var cursor = await recovered.GetRunCursorAsync("run-1", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task AppendSnapshotsEventBeforeWaitingForTheWriterGate()
    {
        ...
        try
        {
            ...
            await injector.Entered.WaitAsync(TimeSpan.FromSeconds(10), cancellationToken: TestContext.Current.CancellationToken);
        ...
        try
        {
            ...
            await using (var faulted = new FileSessionStore(path, faultOptions))
            {
                ...
                await Assert.ThrowsAsync<JournalFaultedException>(
                    () => faulted.GetRunCursorAsync("run-1", cancellationToken: TestContext.Current.CancellationToken).AsTask());
            }
        ...
        try
        {
            ...
            await using (var recovered = new FileSessionStore(path))
            {
                var checkpoint = Assert.Single(
                    await recovered.ReadRunAsync("run-1", TestContext.Current.CancellationToken));
    ...
    public async Task AtomicBatchRecoversEveryEventWithContiguousCursors()
    {
        ...
        try
        {
            ...
            await using (var recovered = new FileSessionStore(path))
            {
                ...
                Assert.Equal(
                    3,
                    (await recovered.GetRunCursorAsync("run-1", cancellationToken: TestContext.Current.CancellationToken)).Revision);
            }
        }
        finally
        {
        ...
        try
        {
            ...
            await using (var store = new FileSessionStore(path))
            {
                ...
                var pending = Assert.Single(
                    await store.ReadPendingOperationsAsync("run-1", cancellationToken: TestContext.Current.CancellationToken));
                ...
                var final = await store.ReconcileReceiptAsync(
                    ...
                    expectedRunRevision: 3, cancellationToken: TestContext.Current.CancellationToken);
            ...
            await using (var recovered = new FileSessionStore(path))
            {
                var operation = await recovered.GetOperationAsync(
                    request.OperationId, cancellationToken: TestContext.Current.CancellationToken);
        ...
        try
        {
            ...
            await Assert.ThrowsAsync<JournalEntryConflictException>(
                () => store.AppendAtomicBatchAsync(
                        ...
                        expectedRunRevision: 4, cancellationToken: TestContext.Current.CancellationToken)
                    .AsTask());
        }
        finally
        {
