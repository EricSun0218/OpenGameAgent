...
public sealed class RunCheckpointLifecycleTests
{
    ...
    public async Task StoreRejectsTerminalCheckpointResurrectionBeforeWrite()
    {
        ...
        try
        {
            ...
            Assert.Equal(
                2,
                (await store.ReadRunAsync(running.RunId, TestContext.Current.CancellationToken)).Count);
        }
        finally
        {
            DeleteJournal(path);
        ...
        long aggregateRevision)
    {
        ...
        try
        {
            ...
            var cursor = await store.GetRunCursorAsync(run.RunId, TestContext.Current.CancellationToken);
        ...
        string mutation)
    {
        ...
        try
        {
            ...
            Assert.Equal(1, cursor.Revision);
            Assert.Single(
                await store.ReadRunAsync(running.RunId, TestContext.Current.CancellationToken));
        }
        finally
        {
            DeleteJournal(path);
    ...
    public async Task TerminalProviderUsageSettlementPreservesTerminalRun()
    {
        ...
        try
        {
            ...
            var recovered = await new RunRecovery(
                    ...
                    store,
                    journal)
                .LoadAsync(running.RunId, TestContext.Current.CancellationToken);
        ...
        try
        {
            ...
            await using (var recovered = new FileSessionStore(legacyPath))
            {
                var cursor = await recovered.GetRunCursorAsync(
                    running.RunId, cancellationToken: TestContext.Current.CancellationToken);
            ...
            Assert.Equal(
                ...
                ReadRun(
                    (await recovered.ReadRunAsync(
                        legacyRunning.RunId,
                        TestContext.Current.CancellationToken))[^1]).State);
            Assert.Equal(
                "current-run-started",
                Assert.Single(
                    await recovered.ReadRunAsync(
                        currentRun.RunId,
                        TestContext.Current.CancellationToken)).EventId);
        }
        finally
        {
            DeleteJournal(path);
