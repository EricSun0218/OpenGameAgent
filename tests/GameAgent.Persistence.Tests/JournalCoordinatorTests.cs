...
public sealed class JournalCoordinatorTests
{
    ...
    public async Task RecoversTranscriptAndReconcilesUnknownOperation()
    {
        ...
        try
        {
            ...
            var recovered = await recovery.LoadAsync(run.RunId, TestContext.Current.CancellationToken);
            ...
            await journal.CommitTransitionAsync(
                recovered.Run,
                RunStates.Completed,
                RuntimeEventKinds.RunCompleted, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task DetachedReconciliationQueryPreventsDuplicateReentry()
    {
        ...
        try
        {
            ...
            var first = recovery.ReconcileAsync(
                    ...
                    cancellation.Token)
                .AsTask();
            await reconciler.Started.Task.WaitAsync(TimeSpan.FromSeconds(10), cancellationToken: TestContext.Current.CancellationToken);
            cancellation.Cancel();
    ...
    public async Task RecoveryUsesLatestBudgetAggregateSnapshot()
    {
        ...
        try
        {
            ...
            await journal.AppendBuiltInDurableAsync(
                ...
                turnId: null,
                attemptId: null, cancellationToken: TestContext.Current.CancellationToken);
            await journal.AppendBuiltInDurableAsync(
                run,
    ...
    public async Task CompletionBatchIsAbsentAfterATornWrite()
    {
        ...
        try
        {
            ...
            {
                var events = await recoveredStore.ReadRunAsync("run-1", TestContext.Current.CancellationToken);
                ...
                var recovered = await new RunRecovery(
                        ...
                        recoveredStore,
                        journal)
                    .LoadAsync("run-1", TestContext.Current.CancellationToken);
    ...
    public async Task ReconciliationRejectsInvalidOrOversizedReceiptsBeforeJournal()
    {
        ...
        try
        {
            ...
            var recovery = new RunRecovery(store, store, journal);
            var recovered = await recovery.LoadAsync(run.RunId, TestContext.Current.CancellationToken);
            var before = (await store.ReadRunAsync(run.RunId, TestContext.Current.CancellationToken)).Count;
    ...
    public async Task RecoveryBackfillsToolTerminalEventAfterFallbackCrashWindow()
    {
        ...
        try
        {
            ...
            await using (var recoveredStore = new FileSessionStore(path))
            {
                ...
                Assert.Empty(recovered!.PendingOperations);
                Assert.Single(
                    await recoveredStore.ReadRunAsync("run-1", TestContext.Current.CancellationToken),
                    item => item.Kind == RuntimeEventKinds.ToolCompleted);
            }
        }
        finally
        ...
        try
        {
                             ...
                             new FileJournalOptions { FaultInjector = fault }))
            {
                ...
                await journal.CommitTransitionAsync(
                    ...
                    turnId: "turn-1",
                    attemptId: "attempt-1", cancellationToken: TestContext.Current.CancellationToken);
                fault.Armed = true;
