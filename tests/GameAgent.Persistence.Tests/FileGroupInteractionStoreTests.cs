...
public sealed class FileGroupInteractionStoreTests
{
    ...
    public async Task RestartRestoresCommittedSessionAndProjection()
    {
            ...
            Assert.Equal(
                ...
                (await store.CreateAsync(
                    Create(
                        ...
                        "session",
                        alice,
                        bob), cancellationToken: TestContext.Current.CancellationToken)).Status);
            Assert.Equal(
                GroupInteractionWriteStatuses.Applied,
        ...
        var session = await recovered.ReadAsync("session", cancellationToken: TestContext.Current.CancellationToken);
        var projection = await recovered.ProjectAsync("session", bob, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task CasConflictsDoNotAppendOrChangeRecoveredState()
    {
        ...
        await using (var store = new FileGroupInteractionStore(path))
        {
            _ = await store.CreateAsync(
                Create("create", "session", alice), cancellationToken: TestContext.Current.CancellationToken);
            ...
            var membershipConflict = await store.AppendAsync(
                Append(
                    ...
                    alice,
                    messageId: "membership-message"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task DuplicateRequestIsIdempotentAndConflictIsStable()
    {
        ...
        await using (var store = new FileGroupInteractionStore(path))
        {
            _ = await store.CreateAsync(
                Create("create", "session", alice), cancellationToken: TestContext.Current.CancellationToken);
            var applied = await store.AppendAsync(request, cancellationToken: TestContext.Current.CancellationToken);
            Assert.Equal(
                GroupInteractionWriteStatuses.Applied,
                applied.Status);
            Assert.Equal(2, store.StoreRevision);
        ...
        await using (var recovered = new FileGroupInteractionStore(path))
        {
            var replay = await recovered.AppendAsync(request, cancellationToken: TestContext.Current.CancellationToken);
            var conflict = await recovered.AppendAsync(
                Append(
                    ...
                    expectedMembershipRevision: 0,
                    alice,
                    messageId: "different-message"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task MembershipHistoryPreservesHistoricalAudienceAfterRestart()
    {
        ...
        {
            _ = await store.CreateAsync(
                Create("create", "session", alice, oldBob), cancellationToken: TestContext.Current.CancellationToken);
            _ = await store.AppendAsync(
                ExplicitAppend(
                    ...
                    expectedMembershipRevision: 0,
                    alice,
                    oldBob), cancellationToken: TestContext.Current.CancellationToken);
        ...
        var newProjection = await recovered.ProjectAsync(
            "session",
            newBob, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task TornTailIsTruncatedToLastCommittedFrame()
    {
                         ...
                         }))
        {
            ...
            _ = await Assert.ThrowsAsync<
                ...
                () => store.ReadAsync("session", cancellationToken: TestContext.Current.CancellationToken).AsTask());
        }
    ...
    public async Task MidFileChecksumCorruptionFailsClosed()
    {
        ...
        await using (var store = new FileGroupInteractionStore(path))
        {
            _ = await store.CreateAsync(
                Create("create", "session", alice), cancellationToken: TestContext.Current.CancellationToken);
            _ = await store.AppendAsync(
                Append("append", "session", 0, 0, alice), cancellationToken: TestContext.Current.CancellationToken);
        }
    ...
    public async Task MutationFrameCapacityRejectsWithoutChangingState()
    {
                         ...
                         options))
        {
            ...
            Assert.Equal(
                0,
                (await store.ReadAsync("session", cancellationToken: TestContext.Current.CancellationToken))!.Revision);
        }
        ...
        Assert.Equal(
            0,
            (await recovered.ReadAsync("session", cancellationToken: TestContext.Current.CancellationToken))!.Revision);
    }
    ...
    public async Task OversizedFrameAndSessionCountFailBeforePublication()
    {
                         ...
                         }))
        {
            ...
            Assert.Null(await frameStore.ReadAsync("large", cancellationToken: TestContext.Current.CancellationToken));
        }
        ...
        _ = await sessionStore.CreateAsync(
            Create("create-one", "one", alice), cancellationToken: TestContext.Current.CancellationToken);
        var sessionError = await Assert.ThrowsAsync<
            FileGroupInteractionStoreCapacityException>(
            () => sessionStore.CreateAsync(
                    Create("create-two", "two", alice), cancellationToken: TestContext.Current.CancellationToken)
                .AsTask());
        Assert.Equal(
            nameof(FileGroupInteractionStoreOptions.MaxSessions),
            sessionError.LimitName);
        Assert.Null(await sessionStore.ReadAsync("two", cancellationToken: TestContext.Current.CancellationToken));
    }
    ...
    public async Task LogByteCapacityRejectsBeforePublishingSnapshot()
    {
                         ...
                         options))
        {
            _ = await store.CreateAsync(
                Create("create", "session", alice), cancellationToken: TestContext.Current.CancellationToken);
            _ = await store.AppendAsync(
                LargeAppend("append-one", "message-one", 0, alice, payload), cancellationToken: TestContext.Current.CancellationToken);
            _ = await store.AppendAsync(
                LargeAppend("append-two", "message-two", 1, alice, payload), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task CancellationBeforeAndWhileWaitingDoesNotPublish()
    {
        ...
        var createTask = Task.Run(
            async () => await blockedStore.CreateAsync(
                Create("create", "session", alice)));
        Assert.True(blocker.Entered.Wait(TimeSpan.FromSeconds(5), cancellationToken: TestContext.Current.CancellationToken));
        try
        {
            using var timeout =
        ...
        var committed = await blockedStore.ReadAsync("session", cancellationToken: TestContext.Current.CancellationToken);
