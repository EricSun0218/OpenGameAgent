...
public sealed class PersistenceCapacityTests
{
    ...
    public async Task JournalEventLimitsAllowExactBoundaryAndRejectNextWrite()
    {
        ...
        try
        {
            ...
            await store.AppendAtomicAsync(Event("event-1", "run-1"));
            await store.AppendAtomicAsync(Event("event-2", "run-1"));
            var beforeLength = new FileInfo(path).Length;
            var beforeCursor = await store.GetRunCursorAsync("run-1");
            ...
            Assert.Equal(
                2,
                (await store.ReadRunAsync("run-1", default)).Count);
    ...
    public async Task JournalTotalLimitCountsEventsAcrossRuns()
    {
        ...
        try
        {
            ...
            await store.AppendAtomicAsync(Event("event-a1", "run-a"));
            await store.AppendAtomicAsync(Event("event-a2", "run-a"));
            await store.AppendAtomicAsync(Event("event-b1", "run-b"));
            var beforeLength = new FileInfo(path).Length;
            var runACursor = await store.GetRunCursorAsync("run-a");
            var runBCursor = await store.GetRunCursorAsync("run-b");

            var error = await Assert.ThrowsAsync<
                JournalCapacityExceededException>(
                () => store.AppendAtomicAsync(
                        Event("event-b2", "run-b"))
                    .AsTask());
    ...
    public async Task JournalPerRunLimitDoesNotConsumeOtherRunCapacity()
    {
        ...
        try
        {
            ...
            await store.AppendAtomicAsync(Event("event-b1", "run-b"));
            await store.AppendAtomicAsync(Event("event-b2", "run-b"));
            Assert.Equal(
                2,
                (await store.ReadRunAsync("run-a", default)).Count);
            Assert.Equal(
                2,
                (await store.ReadRunAsync("run-b", default)).Count);
        }
        finally
        {
            DeleteContainingDirectory(path);
    ...
    public async Task JournalBatchCapacityFailureHasNoDurableSideEffects()
    {
        ...
        try
        {
            ...
            Assert.Equal(beforeLength, new FileInfo(path).Length);
            await AssertCursorAsync(store, "run-1", beforeCursor);
            Assert.Single(await store.ReadRunAsync("run-1", default));
    ...
    public async Task JournalBatchPayloadIsBoundedBeforeFrameMaterialization()
    {
        ...
        try
        {
            ...
            var cursor = await store.GetRunCursorAsync("run-1");
    ...
    public async Task JournalFramePayloadLimitAllowsExactBoundaryAndRejectsLowerReopen()
    {
        ...
        try
        {
            ...
            {
                await store.AppendAtomicAsync(Event("event-a", "run-1"));
            }
            ...
            Assert.Single(await recovered.ReadRunAsync("run-1", default));
        }
        finally
        {
    ...
    public async Task OversizedJournalSnapshotsAreRejectedBeforeFullMaterialization()
    {
        ...
        try
        {
            ...
            var singleAppend = store.AppendAtomicAsync(singleEvent);
            ...
            var batchAppend = store.AppendAtomicBatchAsync(batch);
            var batchAllocation =
        ...
        try
        {
            ...
            for (var index = 0; index < 8; index++)
            {
                ...
                _ = await Assert.ThrowsAsync<
                    JournalCapacityExceededException>(
                    () => store.AppendAtomicAsync(
                            Event($"rejected-single-{index}", runId))
                        .AsTask());
                _ = await Assert.ThrowsAsync<
                    JournalCapacityExceededException>(
                    () => store.AppendAtomicBatchAsync(
    ...
    public async Task JournalRestartRejectsEachExceededCapacityWithoutRewriting()
    {
        ...
        try
        {
            ...
            Assert.Equal(
                2,
                (await recovered.ReadRunAsync("run-1", default)).Count);
        }
        finally
        {
            DeleteContainingDirectory(path);
    ...
    public async Task RunRecoveryLimitRejectsBeforeLedgerOrJournalSideEffects()
    {
        ...
        var error = await Assert.ThrowsAsync<
            ...
            () => recovery.LoadAsync("run-1", default).AsTask());
    ...
    public async Task RunRecoveryAllowsExactConfiguredEventBoundary()
    {
        ...
        var recovered = await recovery.LoadAsync("run-1", default);
