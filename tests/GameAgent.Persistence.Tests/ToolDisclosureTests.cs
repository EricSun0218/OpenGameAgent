...
public sealed class ToolDisclosureTests
{
    ...
    public async Task SearchControlsEnforceResultQueryAndPerTurnCaps()
    {
        ...
        Assert.DoesNotContain(
            await rig.Store.ReadRunAsync(outcome.Run.RunId, TestContext.Current.CancellationToken),
    ...
    public async Task ActivationStateIsIsolatedPerRun()
    {
        ...
        var first = await rig.Runtime.RunAsync(
            new DurableRunRequest { Run = Run("agent-a") }, cancellationToken: TestContext.Current.CancellationToken);
        var second = await rig.Runtime.RunAsync(
            new DurableRunRequest { Run = Run("agent-b") }, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task RequiredDeferredToolIsCallableSameTurnAndOptionalStaysHidden()
    {
        ...
        Assert.Single(((SucceedingHost)rig.Host).Requests);
        var events = await rig.Store.ReadRunAsync(outcome.Run.RunId, TestContext.Current.CancellationToken);
