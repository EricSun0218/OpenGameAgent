...
public sealed class DynamicSkillRuntimeTests
{
    ...
    public async Task ModelCanSearchStructuredCatalogActivateExactlyAndUseSkillNextTurn()
    {
        ...
        var events = await rig.Store.ReadRunAsync(run.RunId, TestContext.Current.CancellationToken);
    ...
    public async Task DynamicActivationSurvivesDurableResume()
    {
        ...
        var resumed = await rig.Runtime.ResumeAsync(
            run.RunId,
            reconciler: new SucceedingReconciler(), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ResolvedSkillContentIsBoundedAndInjectedAsNonAuthoritativeContext()
    {
        ...
        var outcome = await rig.Runtime.RunAsync(
            Request(run, "world-context"), cancellationToken: TestContext.Current.CancellationToken);
        ...
        var snapshot = Assert.Single(
            await rig.Store.ReadRunAsync(run.RunId, TestContext.Current.CancellationToken),
            item => item.Kind == RuntimeEventKinds.TurnSnapshot);
    ...
    public async Task ResolverFailureDoesNotLeakReferenceOrHostError()
    {
        ...
        var outcome = await rig.Runtime.RunAsync(
            Request(run, "private-context"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task OversizedResolverResultIsObservableAndNeverInjected()
    {
        ...
        var outcome = await rig.Runtime.RunAsync(
            Request(Run(), "bounded-content"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task DeclaredContentWithoutResolverFailsClosedBeforeProvider()
    {
        ...
        var outcome = await rig.Runtime.RunAsync(
            Request(Run(), "resolver-required"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task DeclaredDigestIsVerifiedAgainstCanonicalResolvedContent()
    {
        ...
        var outcome = await rig.Runtime.RunAsync(
            Request(Run(), "digest-bound"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task NonCooperativeResolverTimesOutWithoutBlockingRunOrShutdown()
    {
        ...
        var outcome = await rig.Runtime.RunAsync(
            Request(Run(), "timeout-bound"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ShutdownCancelsAndBoundsResolverThatIgnoresCancellation()
    {
        ...
        var run = rig.Runtime.RunAsync(
                Request(Run(), "shutdown-bound"), cancellationToken: TestContext.Current.CancellationToken)
            .AsTask();
        await resolver.Started.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
        var started = DateTimeOffset.UtcNow;
    ...
    public async Task BlockingResolverCancellationRemainsBoundedAndTracked()
    {
        ...
        {
            var outcome = await rig.Runtime.RunAsync(
                Request(Run(), "blocking-cancellation"), cancellationToken: TestContext.Current.CancellationToken);
            ...
            var started = DateTimeOffset.UtcNow;
            await rig.Runtime.WaitForShutdownDrainAsync(cancellationToken: TestContext.Current.CancellationToken)
                .AsTask()
    ...
    public async Task RelatedReferenceFanOutIsRejectedBeforeTraversal()
    {
        ...
        var outcome = await rig.Runtime.RunAsync(
            Request(Run(), "fan-out-bound"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ResolverCancellationCapacityIsIsolatedAndSharedAcrossRuntimes()
    {
        ...
        try
        {
            var firstResolution = firstRuntime.ResolveAsync(
                    ...
                    CancellationToken.None)
                .AsTask();
            await firstResolver.Started.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
            var firstFailure =
            ...
            var secondResolution = secondRuntime.ResolveAsync(
                    ...
                    CancellationToken.None)
                .AsTask();
            await secondResolver.Started.WaitAsync(TimeSpan.FromSeconds(2), cancellationToken: TestContext.Current.CancellationToken);
            var secondFailure =
                await Assert.ThrowsAsync<SkillContentResolutionException>(
                    () => secondResolution);
            Assert.Equal(
