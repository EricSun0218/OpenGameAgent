...
public sealed class ProviderDispatchRecoveryTests
{
        ...
        string mutation)
    {
        var directory = Path.Combine(
            Path.GetTempPath(),
            "game-agent-completion-evidence-tests",
            Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(directory);
        try
        {
        ...
        bool partial)
    {
        ...
        try
        {
            ...
            await store.AppendAtomicAsync(dispatch, run.Revision, cancellationToken: TestContext.Current.CancellationToken);
        ...
        try
        {
            ...
            var dispatchAppend = await store.AppendAtomicAsync(
                ProviderEvent(
                    ...
                    "provider-1",
                    clock.UtcNow),
                run.Revision, cancellationToken: TestContext.Current.CancellationToken);
        ...
        bool emitToolCall)
    {
        ...
        try
        {
            ...
            var beforeRecovery = await store.ReadRunAsync(run.RunId, TestContext.Current.CancellationToken);
            ...
            var repeated = await recoveredRuntime.ResumeAsync(run.RunId, cancellationToken: TestContext.Current.CancellationToken);
            Assert.Equal(RunStates.Failed, repeated.Run.State);
            Assert.Equal(0, secondProvider.CallCount);

            var afterRecovery = await store.ReadRunAsync(run.RunId, TestContext.Current.CancellationToken);
            Assert.Single(
                afterRecovery,
