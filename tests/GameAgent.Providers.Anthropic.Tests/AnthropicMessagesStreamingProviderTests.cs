...
public sealed class AnthropicMessagesStreamingProviderTests
{
    ...
    public async Task StreamsTextAndAccountsCumulativeCacheUsageExactly()
    {
        ...
        var events = await CollectAsync(provider.StreamAsync(Request(), cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task EncodesNativeMessageAndToolBlocksInRequiredOrder()
    {
        ...
        _ = await CollectAsync(provider.StreamAsync(request, cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task RejectsDefaultReasoningBudgetAbovePerRequestOutputLimit()
    {
        ...
        var error = await Assert.ThrowsAsync<ProviderException>(
            () => CollectAsync(provider.StreamAsync(request, cancellationToken: TestContext.Current.CancellationToken)));
    ...
    public async Task AdaptiveBudgetAndUnsupportedDisableFailBeforeTransport()
    {
        ...
        var budgetError = await Assert.ThrowsAsync<ProviderException>(
            () => CollectAsync(provider.StreamAsync(budgetRequest, cancellationToken: TestContext.Current.CancellationToken)));
        Assert.Equal(
            "provider_inference_control_unsupported",
            budgetError.Code);
        ...
        var disabledError = await Assert.ThrowsAsync<ProviderException>(
            () => CollectAsync(provider.StreamAsync(disabledRequest, cancellationToken: TestContext.Current.CancellationToken)));
        Assert.Equal(
            "provider_inference_control_unsupported",
            disabledError.Code);
        Assert.Null(transport.LastRequest);
    ...
    public async Task AcceptsRedactedThinkingWithoutExposingIt()
    {
        ...
        var events = await CollectAsync(
            CreateProvider(new FakeTransport(stream))
                .StreamAsync(Request(), cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task AccountsFinalUsageBeforeTruncatedMessageStop()
    {
        ...
        await using var enumerator = provider
            .StreamAsync(Request(), cancellationToken: TestContext.Current.CancellationToken)
            .GetAsyncEnumerator(cancellationToken: TestContext.Current.CancellationToken);
